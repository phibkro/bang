...
root), auto-`use`d (ADR-0097): no `import`/`use` line needed. They are `let rec` bindings,
...
names a program actually mentions (a syntactic scan, ADR-0097 — this is a FUEL discipline, not
...
- `match 'Z' { Char(n) -> n }` ⟹ `90`
- `match (($concat) \"foo\" \"bar\") { SNil -> 0, SCons(c, t) -> match c { Char(n) -> n } }` ⟹ `102`  — the auto-`use`d STDLIB (free in every program, no import needed — `Prelude.bang`, ADR-0097): `concat`.
