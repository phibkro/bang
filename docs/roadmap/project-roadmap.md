...
| **now** | build-tool consumer measures reverse-dependency closure over the resolver DAG | exact affected sets price topology-aware rebuild work without pretending module counts are latency | validate the public facts; measure structural fanout; keep hashes, persistence, and scheduling out until the consumer evidence is interpreted |
