...
| **now** | spreadsheet tracers plus a 100-line profiled invoice DAG | static dependencies and runtime evaluation are separate observations; 104 stable formulas currently execute 401 calls per force | converge the measurement, then choose cache semantics explicitly; do not infer runtime cost from edges or call uncached pull incremental |
