...
| **now** | compiler query separates the resolved core result from checked per-module public interfaces | body/private edits can preserve an interface while moving core; public signature/shape edits move it; global effect labels still couple otherwise unchanged modules | keep both 64-bit probes cache-unsafe and the interface explicitly not separate-compilation-ready; stabilize cross-module identities before designing a real code artifact/store |
