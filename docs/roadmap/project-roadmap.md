...
| **now** | 100-line invoice baseline plus observation-scoped Memo realization | shared work falls from 401 to 203 formula calls while freshness survives; retained handler memory reproduces the stale-cache failure | preserve call-by-name force; make cache lifetime explicit; do not generalize one Int entry into cross-observation invalidation or a cache API yet |
