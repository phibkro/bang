...
| **now** | an external interface-diff consumer joins complete checked exports to validated resolver fanout | body-only edits preserve; signature/add/remove/topology edits produce scoped recheck candidates; a cross-module public-law edit stops as unattributable | keep actual skips/reuse false; resolve module-owned public-law identity in a separate schema increment, then isolate lowered module identity/body/link constraints before any store |
