...
| **now** | compiler query exposes a resolved-program elaborated-core fingerprint after fanout measurement | formatting/comment and alpha edits preserve the result while semantic edits move it; current whole-program scope reveals the missing per-module artifact boundary | keep the 64-bit probe explicitly cache-unsafe; design the module-result/interface seam before choosing cryptographic storage or scheduling machinery |
