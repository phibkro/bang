...
| **now** | resolver DAG exported through the compiler fact graph | tool authors can observe exact transitive module topology before incremental compilation is designed | project the existing resolver state; expose no paths or hashes; measure invalidation fanout before caching |
