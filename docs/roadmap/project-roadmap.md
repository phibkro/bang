...
| **now** | multi-operation capability passed through a function to concrete Wasm | exact runtime operation dispatch and per-clause update behavior remove the backend workaround exposed by scoped reuse | preserve module-local identity and fail-loud misses; measure before indexing; do not mistake internal ids for a linking ABI |
