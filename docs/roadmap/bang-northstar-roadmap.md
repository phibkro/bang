...
Status: K0 locked · K1 done · K2 done (the pure core calculated + proven across 4 increments) · **K3 in progress** (Throws + State calculated + proven; **Throws now composed with the closure/CBN core** — `CalcCBNEff`, proven, with the forcing-can-raise interaction; State-over-closures and multi-shot reification still open). Everything proven, **zero `sorry`s**. Live detail: `AGENTS.md` "Current playhead".
...
| **K3** | effects on the machine | per-effect calculated machines: **Throws** → handler stack + unwinding (`CalcEff`), **State** → threaded register (`CalcSt`), both **proven**; **Throws composed with the closure/CBN core** (`CalcCBNEff`, proven — forcing-can-raise, re-throw at the meta-call boundary); handler stack → config, effect ops → instructions; STM axiomatized | correctness over the effect monad | effectful BANG end-to-end | 🟡 Throws+State proven · **Throws∘closures proven** · State∘closures + reification open |
