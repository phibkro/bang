...
Status: K0 locked · K1 done · K2 done (the pure core calculated + proven across 4 increments) · **K3 in progress** (Throws + State calculated + proven, and **both now composed with the closure/CBN core** — `CalcCBNEff` (Throws, re-throw at the boundary) + `CalcCBNSt` (State, register threads cleanly — no flatten); only multi-shot reification + multi-effect composition still open). Everything proven, **zero `sorry`s**. Live detail: `AGENTS.md` "Current playhead".
...
| **K3** | effects on the machine | per-effect calculated machines: **Throws** → handler stack + unwinding (`CalcEff`), **State** → threaded register (`CalcSt`), both **proven**; **both composed with the closure/CBN core** — `CalcCBNEff` (Throws: forcing-can-raise, re-throw at the boundary) + `CalcCBNSt` (State: register threads through the nested meta-runs, no flatten), proven; handler stack → config, effect ops → instructions; STM axiomatized | correctness over the effect monad | effectful BANG end-to-end | 🟡 Throws+State proven · **both composed with closures, proven** · multi-effect + reification open |
