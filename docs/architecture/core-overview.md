...
graph TD
  subgraph tier_Frontend["Frontend — text → IR"]
    Frontend_NamedCore["Frontend.NamedCore<br/>380L · fan-in 0"]
    Surface["Surface<br/>760L · fan-in 2"]
    Surface_Trait["Surface.Trait<br/>412L · fan-in 0"]
  end
  subgraph tier_Core["Core — IR · typing · KERNEL (the narrow waist)"]
    Core["Core<br/>345L · fan-in 7"]
    EffectRow["EffectRow<br/>195L · fan-in 1"]
    Mult["Mult<br/>77L · fan-in 2"]
    Operational["Operational<br/>871L · fan-in 10"]
    Syntax["Syntax<br/>345L · fan-in 6"]
  end
  subgraph tier_Backend["Backend — calc-VM → WasmFX"]
    CalcVM["CalcVM<br/>4321L · fan-in 2"]
    Compile["Compile<br/>3311L · fan-in 1"]
  end
  subgraph tier_Meta["Meta — soundness + logical relations (the proofs)"]
    Compat["Compat<br/>2281L · fan-in 1"]
    LR["LR<br/>1135L · fan-in 2"]
    Metatheory["Metatheory<br/>2561L · fan-in 2"]
  end
  subgraph tier_Apex["Apex — verification spine + gate"]
    Audit["Audit<br/>47L · fan-in 0"]
    Distribution["Distribution<br/>63L · fan-in 0"]
    Spec["Spec<br/>287L · fan-in 2"]
  end
  subgraph tier_Regress["Regress — build-gated witnesses"]
    CapEscapeWitness["CapEscapeWitness<br/>69L · fan-in 0"]
    LWRegress["LWRegress<br/>88L · fan-in 1"]
  end
  subgraph tier_Reify["Reify — ADR-0015 paused reification frontier (standalone island)"]
    CalcReify["CalcReify<br/>263L · fan-in 2"]
    CalcReifyRef["CalcReifyRef<br/>157L · fan-in 1"]
    CalcReifySim["CalcReifySim<br/>1434L · fan-in 0"]
  end
  Audit --> Spec
  Audit --> Surface
  Audit --> CalcVM
  CalcReifyRef --> CalcReify
  CalcReifySim --> CalcReify
  CalcReifySim --> CalcReifyRef
  CalcVM --> Operational
  CapEscapeWitness --> Operational
  CapEscapeWitness --> Mult
  CapEscapeWitness --> LWRegress
  Compat --> Core
  Compat --> Syntax
  Compat --> Operational
  Compat --> LR
  ...
  Core --> EffectRow
  Distribution --> Spec
  Frontend_NamedCore --> Operational
  LR --> Core
  LR --> Syntax
  LR --> Operational
  LWRegress --> Operational
  LWRegress --> Mult
  Metatheory --> Core
  Metatheory --> Syntax
  Metatheory --> Operational
  Operational --> Core
  Operational --> Syntax
  Spec --> Core
  Spec --> Syntax
  ...
  Spec --> Compat
  Surface --> Operational
  Surface_Trait --> Surface
  Syntax --> Core
```

| module | tier | LOC | fan-in |
|---|---|---|---|
| `Operational` | Core | 871 | 10 |
| `Core` | Core | 345 | 7 |
| `Syntax` | Core | 345 | 6 |
| `CalcReify` | Reify | 263 | 2 |
| `CalcVM` | Backend | 4321 | 2 |
| `LR` | Meta | 1135 | 2 |
| `Metatheory` | Meta | 2561 | 2 |
| `Mult` | Core | 77 | 2 |
| `Spec` | Apex | 287 | 2 |
| `Surface` | Frontend | 760 | 2 |
| `CalcReifyRef` | Reify | 157 | 1 |
| `Compat` | Meta | 2281 | 1 |
| `Compile` | Backend | 3311 | 1 |
| `EffectRow` | Core | 195 | 1 |
| `LWRegress` | Regress | 88 | 1 |
| `Audit` | Apex | 47 | 0 |
| `CalcReifySim` | Reify | 1434 | 0 |
| `CapEscapeWitness` | Regress | 69 | 0 |
| `Distribution` | Apex | 63 | 0 |
| `Frontend.NamedCore` | Frontend | 380 | 0 |
| `Surface.Trait` | Frontend | 412 | 0 |
