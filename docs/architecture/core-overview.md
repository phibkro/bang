...
```mermaid
graph TD
  subgraph Frontend["Frontend — text → IR"]
    Frontend_NamedCore["Frontend.NamedCore
380L · fan-in 0"]
    Surface["Surface
760L · fan-in 2"]
    Surface_Trait["Surface.Trait
412L · fan-in 0"]
  end
  subgraph Core["Core — IR · typing · KERNEL (the narrow waist)"]
    Core["Core
345L · fan-in 7"]
    EffectRow["EffectRow
195L · fan-in 1"]
...
345L · fan-in 6"]
  ...
  subgraph Backend["Backend — calc-VM → WasmFX"]
    CalcVM["CalcVM
4321L · fan-in 2"]
    Compile["Compile
3311L · fan-in 1"]
  end
  subgraph Meta["Meta — soundness + logical relations (the proofs)"]
    Compat["Compat
2281L · fan-in 1"]
    LR["LR
1135L · fan-in 2"]
...
2561L · fan-in 2"]
  ...
  subgraph Apex["Apex — verification spine + gate"]
    Audit["Audit
47L · fan-in 0"]
...
287L · fan-in 2"]
  ...
  subgraph Regress["Regress — build-gated witnesses"]
    CapEscapeWitness["CapEscapeWitness
69L · fan-in 0"]
    LWRegress["LWRegress
88L · fan-in 1"]
  end
  subgraph Reify["Reify — ADR-0015 paused reification frontier (standalone island)"]
    CalcReify["CalcReify
263L · fan-in 2"]
    CalcReifyRef["CalcReifyRef
157L · fan-in 1"]
    CalcReifySim["CalcReifySim
1434L · fan-in 0"]
  end
  Audit --> Spec
  Audit --> Surface
