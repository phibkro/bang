...
- **Typing:** `HasVTy` (values) / `HasCTy` (computations), in `Bang/Core/Typing.lean`. A `handle` rule's
...
- `perform (vcap n ℓ) op v` dispatches by **identity match**: `splitAtId K n` walks the stack for the
  handler frame whose minted id is exactly `n` (`Bang/Core/Semantics.lean`). Found → resolve; **not found →
...
`Source.step` / `Source.eval` (`Bang/Core/Semantics.lean`): a CK machine over `Config = Nat × EvalCtx × Comp`
...
(`Bang/Backend/AbstractMachine.lean` ~4320 LOC, `Bang/Backend/Wasm.lean` ~3310 LOC) but is **deferred RED**: **route-B**
...
- **Step-indexed logical relation** (Biernacki et al. POPL'18) — `Bang/Meta/LR.lean` + `Bang/Meta/BinaryLR.lean`:
