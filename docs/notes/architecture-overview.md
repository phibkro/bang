...
2. **The type system lives at Core — and nowhere else.** `HasVTy`/`HasCTy` (`Bang/Syntax.lean`), intrinsic
   and resource-enforcing. It carries three things: the CBPV value/computation split (`VTy`/`CTy`,
   `Bang/Core.lean`), the **effect row** (`EffRow = Finset Label`, a set — idempotent join, invariant #2),
...
4. **Two hops, two methods, no optimizer** (invariant #7: performance is second-class):
   - Core → CalcVM: **Bahr–Hutton calculation** (`Bang/CalcVM.lean`).
   - CalcVM → WasmFX: **annotated forward simulation** — `compile_forward_sim` (`Bang/Compile.lean`,
...
| | what |
|---|---|
| **LANDED** | Core kernel + type system · reference CK-machine (`Source.eval`, `Bang/Operational.lean`) · CalcVM CBPV-spine calculation · Compile/WasmFX statements · untyped Surface tracer (`Bang/Surface.lean`) |
...
| **FRONTIER** | `Bang/CalcReify.lean` — multi-shot / non-tail continuation reification (post-v1) |
