...
- **Status:** Accepted — scoped by hktscope (2026-07-07), the mono-additive core VALIDATED by Stage C (`7d887c2`). **Implementation staged (PATH bite-3):** Stage C DONE — concrete-use `impl Functor for Option`, `fmap inc (Some 5) : Option Int ⇒ 6` runs via monomorphization (Case A = the bite-2 `bfnWrapper` move keyed on a ctor name), confirming HKT is mono-additive. Stages A+B (`IVTy.tcon1` + kinds-as-arity + injectivity unify) are BUILT + unit-tested by synthetic guards but **NOT yet wired into the checker main flow** — they are the additive substrate for Case B (abstract-over-f), the next rung. **Case-B seam (hktA):** wiring the substrate requires the checker to KEEP named applications as `tcon1` rather than monomorphizing them to `mu` at `embVInst`/`resolveTy` (the `mu` erases the ctor name injectivity needs). Stage D (Monad/Parser/laws) pending.
- **Date:** 2026-07-07
