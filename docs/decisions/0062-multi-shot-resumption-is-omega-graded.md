...
1. **The built-in handlers are already one-shot.** `dispatchOn` (`Bang/Core/Semantics.lean:329`) reinstalls and
...
2. **QTT carries no order.** `Bang/Core/Grade.lean` provides `CommSemiring QTT` and nothing else; the only
   grade-massaging is `q_or_1 q := if q = 0 then 1 else q` (`Bang/Core/Typing.lean`), a floor *function*, not a `≤`.
...
3. **The `+` that combines resumption counts is non-idempotent** (`1 + 1 = ω`, `Bang/Core/Grade.lean`). The only
...
`krelS_append` (`Bang/Meta/BinaryLR.lean:1146`) recurses on `termination_by (m, Kᵢ.length)`, whose `decreasing_by`
...
1. Add the `gradeOf` conjunct (`gradeOf_append` + `gradeOf_handleF_oneShot`) to `krelS_append`'s resume clause;
   it discharges by `one_mul`. The conjunct rides `dispatchOn_append_outer` (`Bang/Meta/BinaryLR.lean:1085`).
...
- Source: `Bang/Core/Grade.lean` (`CommSemiring QTT`), `Bang/Core/Typing.lean` (`q_or_1`), `Bang/Core/Semantics.lean`
  (`dispatchOn`), `Bang/Meta/BinaryLR.lean` (`krelS_append`, `dispatchOn_append_outer`), `Bang/Core/IR.lean`
