...
Proposed (2026-07-10, lane lrfresh design consult). **The operator rules; this ADR presents.**
The evidence base is krnl3's four machine-checked wall witnesses (all elaborate against main, the
banked lemmas axiom-clean) + this lane's code-read of the frozen DEF block + the `HasStack`
constructor. No live-file change is made by this consult; the recommendation is a carrier SHAPE and
a slice plan for a follow-up proof-layer unit under kernel-engineer + operator sign-off.
