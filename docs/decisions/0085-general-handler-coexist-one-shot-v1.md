...
- **Status:** Proposed — design-first pass (2026-07-07); staged implementation begun.
  ...
  - **Stage 2 RESOLUTION (2026-07-09, superseding the block above — kept as the honest finding it was): LANDED census-clean at `0c6ba99`+`6413281`.** ADR-0087 dissolved the capsH wall by REPRESENTATION (finite `List (OpId × Comp)` clauses — cap-enumeration structural, NO VcapFree-clause invariant threaded; the fix sketched above superseded), and the residual rung-2 wall (NoResume on the clean `run_evalD` path) closed via the `NoCustomFrame K` scaffolding premise (named expiry: Stage 4). The census gate that blocked `gh44s2` PASSES with dispatch real. Full verdict: ADR-0087 §Status. The running-user-effect milestone is UNGATED; the arc proceeds 3(typing)→4(calc-machine: derive the custom arm, drop `CustomFree` + `NoCustomFrame`)→5(LR)→6(soundness)→7(surface).
  - **Stage 4 note (ADR-0086 interaction, 2026-07-09):** the ◊5 headline `compile_forward_sim` now carries a TEMPORARY `CustomFree c` premise (ADR-0086) because Stage-1's `evalD custom = none` inverts for the completeness direction (the hypothesis is a kernel run, not `evalD = some`). When **Stage 4 derives the custom machine arm** (`evalD` defined on custom), that premise is DROPPED from `compile_forward_sim` and the internal `evalD_complete_gen` — a premise-drop is a consumer-safe strengthening, no second re-freeze ritual (cite ADR-0086 when doing so). The `CustomFree`/`CFComp`/`CFStore`/`CFHeap` machinery (`Bang/Core/Freshness.lean` + `Bang/Backend/U5bComplete.lean`) is the scaffolding this drop retires.
- **Date:** 2026-07-07
