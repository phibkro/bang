...
**Workflow automation (2026-06-28 — climbed convention→enforcement where unambiguous):** the
#40 cache-get-in-worktree footgun is now a STRUCTURAL guard — `tools/hooks/pretool-gate-guard.sh`
(PreToolUse, registered in `.claude/settings.json`) denies `lake exe cache get` from inside a
linked worktree (detected by `git-dir ≠ git-common-dir`, not a path string — so it can't
false-positive on a quoted path). The axiom-gate procedure is packaged as the **`/gate`** skill
(`.claude/skills/gate/`); read-only refute-first verification as the **`lean-proof-auditor`**
agent. The grep-traps above deliberately stay GUIDANCE, NOT a hook block: a hook can only
string-match the command, which false-positives on the pattern quoted inside a message (the first
draft blocked its own introducing commit) — enforce the unambiguous, guide the heuristic.
...
- **Orientation-doc proof-state** → **GENERATE (tree-aware)** via `gen-proof-state.py` — derives the
  axiom-clean/sorry/sha block in CONTEXT.md from `Audit.lean` + `burndown.sh` + `git`; `--lean-root`
  reads the proof tree, `--check` skips until `Audit.lean` builds (fully live post tree-reunification).
- **PATH lifecycle** → **TEST** via `check-paths.sh` (every active `paths/PATH-*.md` reachable from
  CONTEXT/ROADMAP; none double-archived). In `just fitness`.
- **Module dependency graph** → **GENERATE + compile-check** via `gen-import-graph.py` (the §2 mermaid
  from the `import` edges; `mmdc` renders it at build-time so a broken graph never lands).

**The survey-residue shortlist is now empty** — every derived fact identified in the 2026-06-24 survey
has climbed to generate-or-test. The maintenance instinct holds: when a NEW derived fact appears, climb
it; don't let it land on survey.
