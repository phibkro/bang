...
#   first-class-multi-operation-cap (exact runtime op dispatch + per-clause update mode).
...
build_and_run first-class-multi-operation-cap # runtime custom-cap dispatch: plain + updating clauses
...
# legs (9×2=18) + default-out (2) + component branch (2) = 22 is invariant across online/offline.
want_total=22
got_total=$((pass + fail))
