...
trap 'rm -f "$holds_tmp" "$bogus_tmp" "$nolaws_tmp" "$trailing_tmp" "$deadimpl_tmp"' EXIT
...
trap 'rm -f "$holds_tmp" "$bogus_tmp" "$nolaws_tmp" "$trailing_tmp" "$deadimpl_tmp" "$directcall_tmp"' EXIT
