#!/usr/bin/env python3
# tool: role=check couples=justfile,tools/run-batteries.sh,tools/test-role-labs.sh,tools/git-hooks/pre-commit,.claude/settings.json runs-in=fitness
...
tools/ rides:
  ...
  (a) every `runs-in=verify` script is REACHABLE from the gate — invoked by a recipe in the
      `verify` dependency chain (transitive), enrolled in run-batteries.sh's `batteries`
      array, OR enrolled in test-role-labs.sh's isolated-snapshot `role_lab_batteries` array.
  (b) every primary or nested battery-array entry names a real script whose header says
      `runs-in=verify` (the arrays are enrollment points; a typo or mis-tag fails loud).
...
ROLE_LAB_BATTERIES = os.path.join(ROOT, "tools", "test-role-labs.sh")
...
def required_scripts():
    ...
    out = subprocess.run(
        ["git", "ls-files", "tools/"], capture_output=True, text=True, cwd=ROOT
    ).stdout.splitlines()
    return sorted(
        f
        for f in out
...
def header_of(path):
    ...
    for line in (
        open(os.path.join(ROOT, path), encoding="utf-8").read().splitlines()[:6]
    ):
...
def parse_recipes(text):
    ...
    for line in text.splitlines():
        ...
        if m and not line.startswith(" "):
            ...
            deps[cur] = [
                d for d in m.group(2).strip().split() if d and not d.startswith("#")
            ]
...
def shell_array(path, name):
    """The whitespace-delimited words in one literal shell array."""
    txt = open(path, encoding="utf-8").read()
    m = re.search(rf"(?m)^{re.escape(name)}=\((.*?)\)", txt, re.DOTALL)
    if not m:
        return None
...
def main():
    ...
    if missing_hdr:
        print("── check-runs-in ──")
        print(
            "FAIL: tools/ scripts missing the `# tool:` header (gen-tools-index owns this too):"
        )
    ...
    verify_tools = tools_invoked_by(
        verify_chain, recipes
    )  # e.g. {selfcheck.mjs, run-batteries.sh, audit.sh}

    bats = shell_array(BATTERIES, "batteries")
    role_lab_bats = shell_array(ROLE_LAB_BATTERIES, "role_lab_batteries")
    if bats is None or role_lab_bats is None:
        print("── check-runs-in ──")
        print("FAIL: could not parse the primary or nested role-lab battery array.")
        return 1
    enrolled = bats + role_lab_bats
    battery_scripts = {f"{b}.sh" for b in enrolled}
    ...
    for s, (role, couples, runs_in, status) in hdrs.items():
        if runs_in == "verify":
            ...
            if base not in reachable:
                errors.append(
                    f"(a) {s} claims runs-in=verify but is NOT reachable from the "
                    f"verify chain (recipes {sorted(verify_chain)}) nor in the "
                    f"primary/nested battery arrays."
                )

    # (b) every batteries entry has a matching runs-in=verify script
    verify_bases = {
        s[len("tools/") :] for s, (r, c, ri, st) in hdrs.items() if ri == "verify"
    }
    for b in enrolled:
        base = f"{b}.sh"
        path = f"tools/{base}"
        if path not in hdrs:
            ...
            errors.append(
                f"(b) batteries entry `{b}` → {path} exists but its header does "
                f"NOT say runs-in=verify (says runs-in={hdrs[path][2]})."
            )

    # (c) every runs-in=hook script is referenced by a hook installation
    pre_src = (
        open(PRECOMMIT, encoding="utf-8").read() if os.path.exists(PRECOMMIT) else ""
    )
    settings_src = (
        open(SETTINGS, encoding="utf-8").read() if os.path.exists(SETTINGS) else ""
    )
    for s, (role, couples, runs_in, status) in hdrs.items():
        if runs_in == "hook":
            base = s[
                len("tools/") :
            ]  # e.g. hooks/session-start.sh, git-hooks/pre-commit
            ...
            referenced = (
                ...
                or leaf in pre_src
                or base in pre_src
                or leaf in settings_src
                or base in settings_src
            )
            if not referenced:
                errors.append(
                    f"(c) {s} claims runs-in=hook but is referenced by neither the "
    ...
    gate_tools = (
        tools_invoked_by(verify_chain | fitness_chain, recipes) | battery_scripts
    )
    for s, (role, couples, runs_in, status) in hdrs.items():
        if status == "deprecated":
            ...
            if in_gate:
                errors.append(
                    f"(d) {s} is status=deprecated but still appears in a gate chain "
    ...
    if errors:
        ...
        return 1
    print(
        f"PASS: {sum(1 for h in hdrs.values() if h[2] == 'verify')} verify-tagged reachable · "
        f"{len(bats)} batteries + {len(role_lab_bats)} isolated role labs all runs-in=verify · "
