...
def render():
    ...
    L.append("MARKER, erased before typing/lowering ever run — zero new semantics). The marker")
    L.append("exists so **`bang fmt` can tell the two apart**: it PRINTS the sugar back for")
    L.append("sugar-parsed input, but does NOT auto-collapse a hand-written nested chain into it —")
    L.append("the two forms stay genuinely distinct through a round-trip, each printing as itself.")
    L.append("")
