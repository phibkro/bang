...
def render():
    ...
    L.append("convention (`let rec` is the only recursion marker; its `… and …` chain is the ONE")
    L.append("mutually-recursive multi-binding form — see below), so sequential-not-recursive is the")
    ...
    L.append("")
    L.append("`let rec` grows a MUTUALLY-RECURSIVE multi-binding form by chaining siblings with `and`:")
    L.append("`let rec f : T1 = e1 and g : T2 = e2 … in body`. Every sibling is in scope in every")
    L.append("sibling's RHS (so `f` may call `g` and `g` may call `f`) — the one place bang's surface")
    L.append("is mutually recursive. A single-binding `let rec` (no `and`) keeps its original")
    L.append("non-mutual shape unchanged; `≥ 1 and` desugars to the `.letRecMultiS` group form.")
    L.append("")
    L.append("Each sibling carries its OWN mandatory `: T ! {row}` ascription (the same rule a")
    L.append("non-structural single `let rec` already needs, ADR-0073): a mutual group's siblings")
    L.append("hand off to each OTHER, not to a strict subterm of their own argument, so neither")
    ...
    L.append("if p1 == p2 then 1 else 0   -- 1: equal, via the derived impl (no hand-written one)")
    L.append("-- ⟹ 1")
    L.append("```")
    L.append("")
    ...
    L.append("hand-written recursive impl rides. The GENERATED impl always uses the carrier's OWN")
    L.append("type-qualified ctor names internally (`IntList_Nil`/`IntList_Cons`, ADR-0099's")
    L.append("`Type_Ctor` form) — so if your carrier's bare ctor names collide with another")
    L.append("co-present type's (e.g. the prelude's built-in `List a = Nil | Cons(a, List a)`,")
    L.append("ADR-0103 Amendment ①), you construct/match VALUES of your own carrier with the SAME")
    L.append("qualified spelling (`IntList_Cons(1, IntList_Nil)`, not bare `Cons(1, Nil)`) — B012")
    ...
    L.append("")
    L.append("<!-- no-gate: a CLI-invocation transcript (command + JSON output), not a bang program -->")
    L.append("```")
    ...
    L.append("")
    L.append("BANG's backend is Wasm 3.0 (ADR-0059): the pure λ + ADT + recursion fragment lowers to")
    L.append("WasmGC. Two CLI verbs expose it — `emit` prints the text, `build` produces the artifact:")
    L.append("")
    L.append("| Verb | Output | Use |")
    L.append("|---|---|---|")
    L.append("| `bang emit <file> [-o out.wat]` | a WasmGC `.wat` MODULE (text) — to stdout or `-o` | inspect / debug the lowering |")
    L.append("| `bang build <file> [-o out.wasm]` | a runnable Wasm binary (default `<stem>.wasm`) | the distribution artifact |")
    L.append("| `bang build <file> --component [--adapter P]` | a WASI component | component-model deployment |")
    L.append("")
    L.append("`build` runs the SAME module-resolved lowering `emit` does, then `wasm-tools parse`")
    L.append("(wat→binary) + `wasm-tools validate`, writing a WASI **command** module — so:")
    L.append("")
    L.append("<!-- no-gate: a shell transcript (CLI invocation + engine run), not a bang program -->")
    L.append("```")
    L.append("bang build examples/json/main.bang -o json.wasm")
    L.append("wasmtime run json.wasm      # → 163  (== the program's Source.eval value)")
    L.append("```")
    L.append("")
    L.append("This is the distribution story (`docs/notes/distribution-survey.md`): the compiled Wasm")
    L.append("module IS bang's static artifact — one file, zero runtime deps, runs on any WASI+GC")
    L.append("engine. `bang build` needs `wasm-tools` on `PATH`; a missing tool or an invalid module")
    L.append("fails LOUD with the tool's own stderr, never a silent or wrong artifact. A program the")
    L.append("GC fragment does not cover (a first-class-capability effect, or host-IO) refuses LOUDLY")
    L.append("at emit time (`EMIT-REFUSED`), exit 1.")
    L.append("")
    L.append("`--component` additionally wraps the module as a WASI **component** via `wasm-tools")
    L.append("component new`. The preview1→WIT adapter is NOT bundled (no nixpkgs WASI adapter), so it")
    L.append("is supplied via `--adapter PATH` or `$BANG_WASI_ADAPTER` (the pinned")
    L.append("`wasi_snapshot_preview1.command.wasm` from wasmtime's releases); absent, `build --component`")
