...
def render():
    ...
    L.append("| `handle e with (Name init) as h { … }` | the PARAMETER-CARRYING form — `init` is threaded internally at install time (see the v1 limitation below: no clause can yet NAME it) |")
    ...
    L.append("")
    L.append("**Known v1 limitations (both tracked, both real gaps in the LANDED surface — not merely")
    L.append("undocumented corners):**")
    L.append("")
    L.append("- **Multi-clause handlers are broken (issue #86).** Every worked example below is")
    L.append("single-op/single-clause; a SECOND clause in the same `with … { … }` block breaks the")
    L.append("FIRST clause's own binder (`unbound variable n` even on a trivial one-op perform once a")
    L.append("second clause exists in the map). A multi-op effect (Reader `ask`+`local`, Logger")
    L.append("`info`+`warn`) does not yet work end to end — stick to single-op effects until this closes.")
    L.append("- **The parameter-carrying form's `init` is NOT clause-nameable (issue #87).** `handle e")
    L.append("with (Name init) as h { op(x) => … }` parses and threads `init` internally, but NO")
    L.append("identifier in a clause body currently resolves to it (an attempted `param`/similar binder")
    L.append("is an unbound-variable error) — the init value is accepted syntactically and then")
    L.append("unreachable. Treat the parameter-carrying form as not-yet-usable in v1; a plain")
    L.append("`effect Name { … }` (no carried param) is the form that works end to end.")
    L.append("")
    L.append("See `examples/handle-custom-tracer/`, `examples/handle-custom-resume/`, and")
