...
def render():
    ...
    L.append("`bang test [<file.bang>]` (issue #60) discovers every law-bearing contract/realization")
    L.append("pair in a decls-only program — traits × impls and effects × named handlers — and")
    ...
    if "handleCustomS" not in surf or "pledgeS" not in surf or "def pDecl" not in surf:
        sys.exit(
            "gen-reference: `handleCustomS`/`pledgeS`/`pDecl` not found in Surface.lean — the "
            "User-defined effects section below is keyed off them (issue #88)."
        )
    L.append("## User-defined effects (ADR-0095, issue #44 Stage 7)")
    L.append("")
    L.append("A user declares a NAMED effect contract (`effect Name { op : ArgTy -> ResTy, …; law … }`),")
    L.append("declares reusable handler REALIZATIONS (`handler H implements Name { … }`), selects one")
    ...
    L.append("| Form | Meaning |")
    L.append("|---|---|")
    L.append("| `effect Name { op : ArgTy -> ResTy, …; law lawName(cap, x): expr }` | declares a named contract; the elaborator allocates a label (`4 + declIndex`, deterministic by decl order) and builds a program-derived op-signature table — the surface analogue of the kernel's `EffSig`. The first law parameter explicitly names the capability; remaining parameters are sampled inputs |")
    L.append("| `handler H implements Name { op(x) => body, … }` | declares a named, reusable static realization; every operation of `Name` must have a clause and no foreign clause is allowed |")
    L.append("| `handle e with H as h` | selects named realization `H`; elaboration expands it to the existing custom-handler form and binds capability `h` around `e` — handlers are not first-class runtime values in this slice |")
    ...
    L.append("| `update op(x) => (resumeValue, nextParam)` | marks one custom clause as parameter-updating: resume with the first value and atomically install the second before the continuation runs (ADR-0114) |")
    ...
    L.append("| `pledge {E₁, E₂, …} in e` | checks that `e`'s inferred row is a subset of the named closed row, retains the actual inferred row, and erases before lowering (ADR-0112) |")
    L.append("")
    ...
    L.append("through the bare word `param`. Ordinary clauses are read-only; an explicit `update`")
    ...
    L.append("`update op(x) => (resumeValue, nextParam)`. Its pair is a protocol envelope, not an")
    L.append("inference from product shape: plain `op(x) => (a, b)` still returns an ordinary product.")
    L.append("The resume component must have the operation result type, the next component must have")
    L.append("the initializer's type, and both must be values. Dispatch installs `nextParam` before")
    L.append("resuming, so later calls through the same capability observe it. The mode is per clause;")
    L.append("plain observations and updating operations may coexist in one handler. An operation")
    L.append("literally named `update` remains plain as `update(x) => body`; the modifier spelling has")
    L.append("two tokens, `update op(…)`. Each operation has exactly one clause: declaring both plain")
    L.append("and updating clauses for the same op is a type error. See `examples/stateful-quota/` for")
    ...
    L.append("")
    L.append("**Effect laws are checked once per named realization.** `bang test` forms the")
    L.append("effect × handler cross product, installs each handler around each law body, and samples")
    L.append("the non-capability parameters. `examples/codec-contract/Codec.bang` therefore yields")
    L.append("four checks: two round-trip laws × `Identity`/`Shift7`. `bang query laws` exports the")
    L.append("same instances with explicit `contract` and `realization` fields.")
    L.append("")
    L.append("**`pledge` makes an effect-row ceiling local and result-type independent.** For example,")
    L.append("`pledge {Audit} in audit.record(41)` is accepted, while adding a `Secret` operation")
    L.append("inside that region is a type error (`pledge violation`). A pure body under the same")
    L.append("ceiling remains pure: the checker returns the body's ACTUAL row rather than widening it")
    L.append("to the allowed row. Every name must resolve to a built-in or declared user effect.")
    L.append("")
    L.append("This is a compile-time assertion, not a grant or runtime filter: the body still needs")
    L.append("the relevant capability, handlers still define admitted operations, and lowering erases")
    L.append("the pledge node. Rows restrict WHICH effect labels may occur; value-level policy such as")
    L.append("filesystem paths or hostnames remains handler-enforced. See `examples/pledged-plugin/`.")
    L.append("")
    L.append("**Value-level resource policy is ordinary handler configuration (ADR-0113).**")
    L.append("`examples/policy-host-allowlist/` runs one unchanged pledged `{Net}` plugin under two")
    L.append("host values carried by `(Net allowed)` and read in the clause as `param`. The row admits")
    L.append("both `connect(7)` and `connect(9)`; the handler decides which request resumes as allowed.")
    L.append("This is complete mediation for checked operations through that capability, not OS isolation")
    ...
    L.append("`check-examples`-gated single-op programs; `examples/stateful-quota/` demonstrates an")
    ...
    L.append("CLI — a Lean script can call `declFactsOf`/`nameRefEdgesOf`/`lawInstancesOf{,Prog}` directly).")
    L.append("")
    ...
    L.append('  "decls": [ { "name": "..", "kind": "let|letRec|fn|trait|impl|data|effect|handler",')
    ...
    L.append('  "laws": [ { "trait": "compat-key", "contract": "..", "realization": ".."|null,')
    L.append('                "law": "..", "params": [".."], "body": "source text" } ],')
    ...
    L.append("a structural summary (ops/ctors/params) for `trait`/`impl`/`data`/`effect`/`handler`, which have no")
    ...
    L.append("")
    L.append("**KNOWN v1 LIMITATION**: a decl's `\"module\"` is `null` unless the CLI layer's own resolution")
    ...
    L.append("field.")
    L.append("Resolver-aware law facts do NOT have that source-text limitation: `lawInstancesOfProg`")
    L.append("walks the merged declarations directly, so imported trait×impl and effect×handler")
    L.append("instances appear in both `dump` and `query laws` (qualified by the module merge).")
    L.append("")
    ...
    L.append("| `laws` | `[<file.bang>]` | every discovered trait×impl and effect×handler law instance |")
