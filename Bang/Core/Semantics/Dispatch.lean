...
def dispatchOn (n : Nat) (op : OpId) (v : Val) :
  ...
  | (Kᵢ, h, Kₒ) =>
      ...
      -- custom (ADR-0085 #44 STAGE 2, ADR-0087 finite rep): ONE-SHOT TAIL-RESUMPTIVE resume (D2). The
      -- `state` mechanism (ADR-0025) generalized — USER clause logic where get/put's hardcode was. Look up
      -- the op's clause (guaranteed `some` here: `idDispatch`'s fail-loud `handlesOp` guard already forced
      -- `(clauses.find? …).isSome`), then RESUME `Kᵢ` with the clause's RESULT and reinstall a deep
