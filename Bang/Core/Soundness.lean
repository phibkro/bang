...
/-- `dispatchOn` returns `some` on every handler that ACTUALLY HANDLES `(ℓ, op)` (ADR-0085/0087: the
built-in triple is total; `custom` is now total too — `handlesOp = true` forces the clause `find?`
lookup `.isSome`, so the one-shot resume `some` is produced). shape: NonEscapeProbe §2. -/
private theorem dispatchOn_isSome (n : Nat) (ℓ : Label) (op : OpId) (v : Val) (X : EvalCtx × Handler × EvalCtx)
  ...
  -- custom (ADR-0085 stage 2, ADR-0087 finite rep): dispatchOn is TOTAL on custom too — `handlesOp = true`
  -- forces the clause `find?` lookup `.isSome`, so it succeeds and the one-shot resume `some` is produced.
  -- Real, additive (the built-in triple's totality generalized to the clause list).
  | custom ℓ' p cl =>
