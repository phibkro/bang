...
-- Axiom gate: plain `#print axioms` (the house witness pattern); the sets are certified on
-- landing via /gate. NOTE: wrapping these in `#guard_msgs (drop info)` is a FALSE gate — the
-- axiom list is info-level whether or not `sorryAx` appears, so (drop info) asserts nothing.
#print axioms binop_body_fixed_grade
#print axioms binop_body_not_at_omega
#print axioms letc_body_not_at_zero
#print axioms letc_body_types_at_one
