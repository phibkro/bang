...
-- alongside the runtime import (per [[lean-eval-reliable-only-compiled]]).
meta import Bang.Backend.Wasm
public import Bang.Backend.Wasm
public import Bang.Core.Freshness

/-! # Witness: `Source.eval … = done` does NOT imply `VcapFree c` (capsC c = []). ADR-0086.

PRECISE CLAIM (does NOT overclaim "unsound-as-stated"): a program with a BURIED (never-forced)
capability literal evaluates to `done`, yet its `capsC` is non-empty AND `FreshCfg (0,[],c)` is
FALSE. So the cap-free premise is **premise-necessary for the evalD-bridge proof ARCHITECTURE /
BRIDGE ROUTE** (its only known route cannot reach such c — FreshCfg fails). It does NOT show
`compile_forward_sim` is FALSE on cWitness: the thunk is never forced, and check (4) below CONFIRMS
the WASM side ALSO completes cWitness — so the HEADLINE is TRUE-but-UNPROVABLE-WITHOUT-THE-PREMISE
on this class. Do-not-weaken regression witness (ADR-0086 Evidence). -/
