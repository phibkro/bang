...
payload, so the cell type is `Val × List (OpId × Comp)` (vs `SStore`'s `Val` and `THeap`'s `List Val`).
...
clause, and INLINE-SERVICES it — runs `subst p (subst (shift v) clause.2)` as a sub-eval against the
SAME store (frame stays live, so nested ops are handled) and resumes with its terminal value. The param
is READ-ONLY in v1 (no in-place update — no `put`-analog), so `CStore` has no `.put`; a put-like
param mutation is a post-v1 concern (ADR-0085 D2/Q27).
...
/-- Find the nearest **custom** frame with IDENTITY `n` and INLINE-SERVICE a user op (ADR-0085 Stage 4,
the user-effect analog of `stateUpdate`/`txnUpdate`). Unlike state/txn, custom's service is NOT an
in-place store write — the clause runs as a re-compiled sub-computation. So this returns the CLAUSE
BODY to run (`subst p (subst (shift v) clause.2)`) together with `hs` UNCHANGED (the frame stays LIVE:
the kernel reinstalls `handleF n`, so nested ops are handled; the param is READ-ONLY in v1). `none` = no
`custom n` frame OR the op is unserviced by its clause list ⇒ the caller falls through to `unwindFind`
(throws path). Mirrors `evalD`'s perform-custom arm (`κ.get? n` → `cls.find? op` → run the clause body). -/
                ...
                | (.raised ℓ' op' w, g1, σ1, τ1, κ1), h =>
                    ...
                    · simp only [if_neg hk, Option.some.injEq, Prod.mk.injEq, Outcome.raised.injEq] at h
                      ...
                      have htriple : Corr σ1 (netEffect hs σ1 τ1) ∧ TCorr τ1 (netEffect hs σ1 τ1)
                        ...
                        obtain ⟨⟨hCr, hTr, hKr, hmutr⟩, _⟩ :=
                          ihR (Comp.subst (Val.vcap g ℓ0) M) (g+1) σ τ κ ℓ' op' w g1 σ1 τ1 κ1 hM (fr0 :: hs)
                            ...
                            (CCorr_install_noncustom fr0 hnc0 hK)
                      ...
                      set fr : HFrame := { id := g, handler := Handler.throws ℓ0, savedCode := c, savedStack := s }
                        with hfrdef
                      obtain ⟨_, kR⟩ := ihR (Comp.subst (Val.vcap g ℓ0) M) (g+1) σ τ κ ℓ' op' w g1 σ1 τ1 κ1 hM (fr :: hs)
                        (Corr_install_nonstate fr hns0 hC) (TCorr_install_nontxn fr hnt0 hT)
                        (CCorr_install_noncustom fr hnc0 hK)
                      have hfwd : throwOutcome F g1 ℓ' op' w (netEffect (fr :: hs) σ1 τ1) = some r := by
...
/-- `splitAtId K n` landing on a `custom ℓ' p cl` frame ⟹ the IDENTITY-keyed custom store has `(p, cl)`
at `n`: `(ctxCustoms K).get? n = some (p, cl)`. Route-B custom mirror of `splitAtId_state_value` — the
lookup is by identity `n`; the frame's label `ℓ'` is immaterial to the store projection. -/
...
theorem NoCustomFrame.not_custom {K : Bang.EvalCtx} (h : NoCustomFrame K)
    {n : Nat} {Kᵢ : Bang.EvalCtx} {ℓ' : Bang.EffectRow.Label} {p : Val} {cl : List (OpId × Comp)}
...
while `evalD` dispatches OP-first (`if get / elif put / elif isTxnOp / else custom`). They DIVERGE on
one class: a `perform (vcap n ℓ) op v` where op is a BUILTIN name (`get`/`put`/TVar-family) but the frame
at `n` is a `custom ℓ p cls` with a clause KEYED that builtin name — the kernel SERVICES the clause,
`evalD` RAISES (probe-confirmed, `scratch/OpPriorityProbe.lean`). `WfCustomOps K` rules this out: every
custom frame in `K` keys only NON-builtin ops. This is the PROOF-SIDE encoding of the SURFACE reservation
(ADR-0092: the elaborator rejects an `effect` decl naming a reserved op `get`/`put`/`raise`/TVar-family
...
theorem WfCustomOps.tail {fr : Bang.Frame} {K : Bang.EvalCtx} (h : WfCustomOps (fr :: K)) :
    WfCustomOps K := by
  cases fr with
  | handleF m hd => cases hd <;> first | exact h | exact h.2
  | letF N => exact h
  | appF w => exact h

/-- Pushing a NON-custom handler/non-frame head preserves `WfCustomOps`. -/
theorem WfCustomOps.cons_noncustom {fr : Bang.Frame} {K : Bang.EvalCtx}
    (hnc : ∀ n ℓ p cls, fr ≠ Frame.handleF n (.custom ℓ p cls)) (h : WfCustomOps K) :
    WfCustomOps (fr :: K) := by
  cases fr with
  | handleF m hd =>
      cases hd with
      | custom ℓ p cl => exact absurd rfl (hnc m ℓ p cl)
      | _ => exact h
  | letF N => exact h
  | appF w => exact h
...
theorem WfCustomOps.updateCtxStates {K : Bang.EvalCtx} (σ : SStore) (h : WfCustomOps K) :
    WfCustomOps (updateCtxStates K σ) := by
  induction K generalizing σ with
  | nil => exact h
  | cons fr K ih =>
    cases fr with
    | handleF m hd =>
        ...
        | throws ℓ0 => exact ih σ h.tail
        | transaction ℓ0 Θ0 => exact ih σ h.tail
        | custom ℓ0 p cl => exact ⟨h.1, ih σ h.2⟩
    | letF N => exact ih σ h
    | appF w => exact ih σ h

/-- `updateCtxTxns` rewrites only txn-frame HEAPS, never clause lists ⇒ `WfCustomOps` rides. -/
theorem WfCustomOps.updateCtxTxns {K : Bang.EvalCtx} (τ : THeap) (h : WfCustomOps K) :
    WfCustomOps (updateCtxTxns K τ) := by
  induction K generalizing τ with
  | nil => exact h
  | cons fr K ih =>
    cases fr with
    | handleF m hd =>
        ...
        | state ℓ0 s0 => exact ih τ h.tail
        | throws ℓ0 => exact ih τ h.tail
        | custom ℓ0 p cl => exact ⟨h.1, ih τ h.2⟩
    | letF N => exact ih τ h
    | appF w => exact ih τ h

/-- `WfCustomOps` rides the full net-effect rebuild (both value passes leave clause lists untouched). -/
...
never names get/put/raise/TVar-family). The direct analog of `VcapFree` (`capsC c = []`). -/
def WfCustomComp (c : Comp) : Prop := ∀ op ∈ customOpsC c, isBuiltinOp op = false

/-- The config-level predicate threaded through `run_evalD` (like `CapLabelCoh`/`FreshCfg`): the context's
custom frames AND the focus's custom handlers are all op-disjoint. -/
...
theorem wfCustomComp_custom_clauses {ℓ : Bang.EffectRow.Label} {p : Val} {cls : List (Bang.OpId × Comp)}
    ...
    ∀ c ∈ cls, isBuiltinOp c.1 = false := by
  intro c hc
  apply h
  simp only [customOpsC, customOpsH]
  -- `++` left-assoc: customOpsH = (customOpsV p ++ cls.map (·.1)) ++ customOpsCls cls; the whole handle =
  -- customOpsH ++ customOpsC M. `c.1 ∈ cls.map (·.1)` ⇒ left of the outer, right of the p-summand.
  exact List.mem_append_left _ (List.mem_append_left _ (List.mem_append_right _ (List.mem_map_of_mem hc)))

/-- A value is `WfCustomVal` when every custom handler REACHABLE inside it (through thunks) keys only
non-builtin ops — the value-level analog of `WfCustomComp` (`customOpsV v` bounds them). Substituting
a `WfCustomVal` into a `WfCustomComp` keeps it `WfCustomComp`. Note a raw `vcap`/`vunit`/`vint`/`vvar`
is trivially `WfCustomVal` (`customOpsV = []`) — the mint's `subst (vcap g ℓ)` is always safe. -/
def WfCustomVal (v : Val) : Prop := ∀ op ∈ customOpsV v, isBuiltinOp op = false

theorem WfCustomVal.vcap {n : Nat} {ℓ : Bang.EffectRow.Label} : WfCustomVal (Val.vcap n ℓ) := by
  intro op hop; simp only [customOpsV] at hop; exact absurd hop (by simp)

-- `customOps{V,C}` are INVARIANT under `shiftFrom` — shifting only renames variables; custom handlers
-- are inert and no subterm is added/removed. Mutual structural recursion on Val/Comp.
mutual
/-- `customOpsV` is invariant under `shiftFrom` (variable renaming; custom handlers inert). -/
theorem customOpsV_shiftFrom (k : Nat) (V : Val) : customOpsV (Val.shiftFrom k V) = customOpsV V := by
  match V with
  | .vcap n ℓ => simp only [Val.shiftFrom, customOpsV]
  | .vthunk c => simp only [Val.shiftFrom, customOpsV, customOpsC_shiftFrom k c]
  | .inl v => simp only [Val.shiftFrom, customOpsV, customOpsV_shiftFrom k v]
  | .inr v => simp only [Val.shiftFrom, customOpsV, customOpsV_shiftFrom k v]
  | .pair a b => simp only [Val.shiftFrom, customOpsV, customOpsV_shiftFrom k a, customOpsV_shiftFrom k b]
  | .fold v => simp only [Val.shiftFrom, customOpsV, customOpsV_shiftFrom k v]
  | .vunit => simp only [Val.shiftFrom, customOpsV]
  | .vint n => simp only [Val.shiftFrom, customOpsV]
  | .vvar i => simp only [Val.shiftFrom]; split <;> simp only [customOpsV]
  termination_by sizeOf V
  decreasing_by all_goals (first | (simp_wf; omega) | simp_wf)
theorem customOpsC_shiftFrom (k : Nat) (M : Comp) : customOpsC (Comp.shiftFrom k M) = customOpsC M := by
  match M with
  | .ret v => simp only [Comp.shiftFrom, customOpsC, customOpsV_shiftFrom k v]
  | .letC M N => simp only [Comp.shiftFrom, customOpsC, customOpsC_shiftFrom k M, customOpsC_shiftFrom (k+1) N]
  | .force v => simp only [Comp.shiftFrom, customOpsC, customOpsV_shiftFrom k v]
  | .lam M => simp only [Comp.shiftFrom, customOpsC, customOpsC_shiftFrom (k+1) M]
  | .app M v => simp only [Comp.shiftFrom, customOpsC, customOpsC_shiftFrom k M, customOpsV_shiftFrom k v]
  | .perform c op v => simp only [Comp.shiftFrom, customOpsC, customOpsV_shiftFrom k c, customOpsV_shiftFrom k v]
  | .handle hd M =>
      cases hd with
      | state ℓ s => simp only [Comp.shiftFrom, Handler.shiftFrom, customOpsC, customOpsH,
          customOpsC_shiftFrom (k+1) M, customOpsV_shiftFrom k s]
      | throws ℓ => simp only [Comp.shiftFrom, Handler.shiftFrom, customOpsC, customOpsH,
          customOpsC_shiftFrom (k+1) M]
      | transaction ℓ Θ =>
          -- txn heap IDENTITY under shiftFrom (closed cells, ADR-0030) ⇒ heap op-keys unchanged.
          simp only [Comp.shiftFrom, Handler.shiftFrom, customOpsC, customOpsH,
            customOpsC_shiftFrom (k+1) M]
      | custom ℓ p cls => simp only [Comp.shiftFrom, Handler.shiftFrom, customOpsC,
          customOpsC_shiftFrom (k+1) M]
  | .case v N₁ N₂ => simp only [Comp.shiftFrom, customOpsC, customOpsV_shiftFrom k v,
      customOpsC_shiftFrom (k+1) N₁, customOpsC_shiftFrom (k+1) N₂]
  | .split v N => simp only [Comp.shiftFrom, customOpsC, customOpsV_shiftFrom k v,
      customOpsC_shiftFrom (k+2) N]
  | .unfold v => simp only [Comp.shiftFrom, customOpsC, customOpsV_shiftFrom k v]
  | .binop op v w => simp only [Comp.shiftFrom, customOpsC, customOpsV_shiftFrom k v, customOpsV_shiftFrom k w]
  | .oom => simp only [Comp.shiftFrom, customOpsC]
  | .wrong s => simp only [Comp.shiftFrom, customOpsC]
  termination_by sizeOf M
  decreasing_by all_goals (first | (simp_wf; omega) | simp_wf)
end

theorem customOpsV_shift {u : Val} : customOpsV (Val.shift u) = customOpsV u :=
  customOpsV_shiftFrom 0 u

-- `customOps{V,C}` under `substFrom` is bounded by the term's op-keys PLUS the substituted value's:
-- `customOpsC (substFrom k u M) ⊆ customOpsC M ++ customOpsV u`. Custom handlers are inert under subst
-- (op-keys unchanged), and a `vvar` may be replaced by `u` (contributing `customOpsV u`).
mutual
/-- `customOpsV (substFrom k u V) ⊆ customOpsV V ++ customOpsV u`. -/
theorem customOpsV_substFrom_subset (k : Nat) (u : Val) (V : Val) :
    ∀ op ∈ customOpsV (Val.substFrom k u V), op ∈ customOpsV V ++ customOpsV u := by
  intro op hop
  match V with
  | .vcap n ℓ => simp only [Val.substFrom, customOpsV, List.not_mem_nil] at hop
  | .vthunk c =>
      simp only [Val.substFrom, customOpsV] at hop ⊢
      exact customOpsC_substFrom_subset k u c op hop
  | .inl v => simp only [Val.substFrom, customOpsV] at hop ⊢; exact customOpsV_substFrom_subset k u v op hop
  | .inr v => simp only [Val.substFrom, customOpsV] at hop ⊢; exact customOpsV_substFrom_subset k u v op hop
  | .pair a b =>
      simp only [Val.substFrom, customOpsV, List.mem_append] at hop ⊢
      rcases hop with ha | hb
      · rcases List.mem_append.mp (customOpsV_substFrom_subset k u a op ha) with h | h
        · exact Or.inl (Or.inl h)
        · exact Or.inr h
      · rcases List.mem_append.mp (customOpsV_substFrom_subset k u b op hb) with h | h
        · exact Or.inl (Or.inr h)
        · exact Or.inr h
  | .fold v => simp only [Val.substFrom, customOpsV] at hop ⊢; exact customOpsV_substFrom_subset k u v op hop
  | .vunit => simp only [Val.substFrom, customOpsV, List.not_mem_nil] at hop
  | .vint n => simp only [Val.substFrom, customOpsV, List.not_mem_nil] at hop
  | .vvar i =>
      -- the ONLY place `u` enters: `i = k` replaces the var by `u`; else a `vvar` (no op-keys).
      simp only [Val.substFrom] at hop
      split at hop
      · exact List.mem_append_right _ hop
      · split at hop <;> simp only [customOpsV, List.not_mem_nil] at hop
  termination_by sizeOf V
  decreasing_by all_goals (first | (simp_wf; omega) | simp_wf)
theorem customOpsC_substFrom_subset (k : Nat) (u : Val) (M : Comp) :
    ∀ op ∈ customOpsC (Comp.substFrom k u M), op ∈ customOpsC M ++ customOpsV u := by
  intro op hop
  match M with
  | .ret v =>
      simp only [Comp.substFrom, customOpsC] at hop ⊢; exact customOpsV_substFrom_subset k u v op hop
  | .letC M N =>
      simp only [Comp.substFrom, customOpsC, List.mem_append] at hop ⊢
      rcases hop with hM | hN
      · rcases List.mem_append.mp (customOpsC_substFrom_subset k u M op hM) with h | h
        · exact Or.inl (Or.inl h)
        · exact Or.inr h
      · rcases List.mem_append.mp (customOpsC_substFrom_subset (k+1) (Val.shift u) N op hN) with h | h
        · exact Or.inl (Or.inr h)
        · exact Or.inr (customOpsV_shift ▸ h)
  | .force v =>
      simp only [Comp.substFrom, customOpsC] at hop ⊢; exact customOpsV_substFrom_subset k u v op hop
  | .lam M =>
      simp only [Comp.substFrom, customOpsC] at hop ⊢
      rcases List.mem_append.mp (customOpsC_substFrom_subset (k+1) (Val.shift u) M op hop) with h | h
      · exact List.mem_append_left _ h
      · exact List.mem_append_right _ (customOpsV_shift ▸ h)
  | .app M v =>
      simp only [Comp.substFrom, customOpsC, List.mem_append] at hop ⊢
      rcases hop with hM | hv
      · rcases List.mem_append.mp (customOpsC_substFrom_subset k u M op hM) with h | h
        · exact Or.inl (Or.inl h)
        · exact Or.inr h
      · rcases List.mem_append.mp (customOpsV_substFrom_subset k u v op hv) with h | h
        · exact Or.inl (Or.inr h)
        · exact Or.inr h
  | .perform c op2 v =>
      simp only [Comp.substFrom, customOpsC, List.mem_append] at hop ⊢
      rcases hop with hc | hv
      · rcases List.mem_append.mp (customOpsV_substFrom_subset k u c op hc) with h | h
        · exact Or.inl (Or.inl h)
        · exact Or.inr h
      · rcases List.mem_append.mp (customOpsV_substFrom_subset k u v op hv) with h | h
        · exact Or.inl (Or.inr h)
  ...
  | .handle hd M =>
      cases hd with
      | state ℓ s =>
          simp only [Comp.substFrom, Handler.substFrom, customOpsC, customOpsH, List.mem_append] at hop ⊢
          rcases hop with hs | hM
          · rcases List.mem_append.mp (customOpsV_substFrom_subset k u s op hs) with h | h
            · exact Or.inl (Or.inl h)
            · exact Or.inr h
          · rcases List.mem_append.mp (customOpsC_substFrom_subset (k+1) (Val.shift u) M op hM) with h | h
            · exact Or.inl (Or.inr h)
            · exact Or.inr (customOpsV_shift ▸ h)
      | throws ℓ =>
          simp only [Comp.substFrom, Handler.substFrom, customOpsC, customOpsH, List.nil_append] at hop ⊢
          rcases List.mem_append.mp (customOpsC_substFrom_subset (k+1) (Val.shift u) M op hop) with h | h
          · exact List.mem_append_left _ h
          · exact List.mem_append_right _ (customOpsV_shift ▸ h)
      | transaction ℓ Θ =>
          -- txn heap is IDENTITY under substFrom (closed cells, ADR-0030) ⇒ the heap op-keys are unchanged;
          -- only the body recurses.
          simp only [Comp.substFrom, Handler.substFrom, customOpsC, customOpsH, List.mem_append] at hop ⊢
          rcases hop with hΘ | hM
          · exact Or.inl (Or.inl hΘ)
          · rcases List.mem_append.mp (customOpsC_substFrom_subset (k+1) (Val.shift u) M op hM) with h | h
            · exact Or.inl (Or.inr h)
            · exact Or.inr (customOpsV_shift ▸ h)
      | custom ℓ p cls =>
          -- custom handler INERT under substFrom ⇒ its op-keys unchanged; only the body recurses.
          simp only [Comp.substFrom, Handler.substFrom, customOpsC, List.mem_append] at hop ⊢
          rcases hop with hh | hM
  ...
  | .split v N =>
      simp only [Comp.substFrom, customOpsC, List.mem_append] at hop ⊢
      rcases hop with hv | hN
      · rcases List.mem_append.mp (customOpsV_substFrom_subset k u v op hv) with h | h
        · exact Or.inl (Or.inl h)
        · exact Or.inr h
      · rcases List.mem_append.mp (customOpsC_substFrom_subset (k+2) (Val.shift (Val.shift u)) N op hN) with h | h
        · exact Or.inl (Or.inr h)
        · exact Or.inr (customOpsV_shift ▸ customOpsV_shift ▸ h)
  | .unfold v =>
      simp only [Comp.substFrom, customOpsC] at hop ⊢; exact customOpsV_substFrom_subset k u v op hop
  | .binop op2 v w =>
      simp only [Comp.substFrom, customOpsC, List.mem_append] at hop ⊢
      rcases hop with hv | hw
      · rcases List.mem_append.mp (customOpsV_substFrom_subset k u v op hv) with h | h
        · exact Or.inl (Or.inl h)
        · exact Or.inr h
      · rcases List.mem_append.mp (customOpsV_substFrom_subset k u w op hw) with h | h
        · exact Or.inl (Or.inr h)
        · exact Or.inr h
  | .oom => simp only [Comp.substFrom, customOpsC, List.not_mem_nil] at hop
  | .wrong s => simp only [Comp.substFrom, customOpsC, List.not_mem_nil] at hop
  termination_by sizeOf M
  decreasing_by
    all_goals simp_wf
    all_goals first
      | omega
      | (next hd => cases hd <;> simp_all <;> omega)
      | (rename_i hd; cases hd <;> simp_all <;> omega)
end

/-- `WfCustomComp` rides `subst` of a `WfCustomVal` (in particular a `vcap` — the mint's substitution). -/
theorem WfCustomComp.subst {M : Comp} {v : Val} (hv : WfCustomVal v) (h : WfCustomComp M) :
    WfCustomComp (Comp.subst v M) := by
  intro op hop
...
theorem WfCustomComp.letC_left {M N : Comp} (h : WfCustomComp (Comp.letC M N)) : WfCustomComp M :=
  fun op hop => h op (by simp only [customOpsC]; exact List.mem_append_left _ hop)
theorem WfCustomComp.letC_right {M N : Comp} (h : WfCustomComp (Comp.letC M N)) : WfCustomComp N :=
  fun op hop => h op (by simp only [customOpsC]; exact List.mem_append_right _ hop)
theorem WfCustomComp.app_body {M : Comp} {v : Val} (h : WfCustomComp (Comp.app M v)) : WfCustomComp M :=
  fun op hop => h op (by simp only [customOpsC]; exact List.mem_append_left _ hop)
...
theorem WfCustomComp.case_left {v : Val} {N₁ N₂ : Comp} (h : WfCustomComp (Comp.case v N₁ N₂)) : WfCustomComp N₁ :=
  fun op hop => h op (by simp only [customOpsC]; exact List.mem_append_left _ (List.mem_append_right _ hop))
theorem WfCustomComp.case_right {v : Val} {N₁ N₂ : Comp} (h : WfCustomComp (Comp.case v N₁ N₂)) : WfCustomComp N₂ :=
  fun op hop => h op (by simp only [customOpsC]; exact List.mem_append_right _ hop)
theorem WfCustomComp.split_body {v : Val} {N : Comp} (h : WfCustomComp (Comp.split v N)) : WfCustomComp N :=
  fun op hop => h op (by simp only [customOpsC]; exact List.mem_append_right _ hop)
...
`close` helper needs for the get/put/txn cases. -/
theorem WfCustomOps.custom_clause_miss {K : Bang.EvalCtx} (h : WfCustomOps K)
    {n : Nat} {Kᵢ : Bang.EvalCtx} {ℓ' : Bang.EffectRow.Label} {p : Val} {cl : List (OpId × Comp)}
    {Kₒ : Bang.EvalCtx} {op : Bang.OpId} (hbi : isBuiltinOp op = true)
    (hsp : Bang.splitAtId K n = some (Kᵢ, Handler.custom ℓ' p cl, Kₒ)) :
    cl.find? (·.1 == op) = none := by
  -- the resolved custom frame is FROM K (`splitAtId`), so its clause list is one of the `WfCustomOps`
  -- conjuncts ⇒ every clause key is non-builtin ⇒ no clause matches the builtin `op`.
  have hcl : ∀ c ∈ cl, isBuiltinOp c.1 = false := by
    induction K generalizing Kᵢ Kₒ with
    | nil => simp [Bang.splitAtId] at hsp
    | cons fr K ih =>
      cases fr with
      ...
      | appF w =>
          ...
          simp only [Prod.mk.injEq] at heq; obtain ⟨_, rfl, rfl⟩ := heq
          exact ih h.tail hsp'
  -- no clause keys `op` (all keys non-builtin, `op` builtin) ⇒ `find?` misses.
  ...
  | succ fe ih =>
    ...
    · -- TERM PART
          ...
          | some oM =>
            ...
            | (.term (.ret v), g1, σ1, τ1, κ1), h =>
                ...
                have hKM' : CCtxCorr κ1 (ctxNetEffect K σ1 τ1) := by
                  ...
                  rw [hKM, ctxCustoms_ctxNetEffect, ctxCustoms_ctxNetEffect]; rfl
      ...
      | handle h0 M =>
          ...
          | custom ℓ0 p0 cls0 =>
              -- route-B INSTALL a custom frame (ADR-0085 Stage 4): MINT id := g, push (id ↦ (p0,cls0)) on κ,
              -- run the SUBSTITUTED body M' = subst (vcap g ℓ0) M at g+1. Custom carries NO state/txn payload,
              -- so CtxCorr/CtxTxnCorr pass through (like throws); CCtxCorr installs/pops the κ entry (like
              -- state does for σ on CtxCorr). Pure install/pop — the clause-service happens at the perform arm
              -- (already proven above). Mirror of the sim custom handle arm (2215) on the `Config.run` side.
              simp only [Handler.label] at h
              cases hM : evalD fe (g+1) σ τ (κ.push g p0 cls0) (Comp.subst (Val.vcap g ℓ0) M) with
              | none => rw [hM] at h; simp at h
              | some oM =>
                ...
                | (.term (.ret v), g1, σ1, τ1, κ1), h =>
                    ...
                    have hKpop : CCtxCorr κ1.tail (ctxNetEffect K σ1 τ1) := by
                      have : CCtxCorr κ1 (Frame.handleF g (Handler.custom ℓ0 p0 cls0) :: ctxNetEffect K σ1 τ1) :=
                        hnetEq ▸ hKM
                      exact CCtxCorr_pop_custom this
                ...
                | (.term (.wrong a), _, _, _, _), h => simp [Option.bind] at h
                | (.raised n' op' w, _, _, _, _), h =>
                    -- body RAISED ⇒ evalD's custom handle arm FORWARDS `.raised` (pop entry) — not `.term t`.
                    -- So the outer `= some (.term t, …)` hypothesis is absurd; the raise-forward lives in the
                    -- RAISED part of run_evalD (via `ihR` on the whole `handle`). Mirror of state/txn here.
                    simp only [Option.bind_some, Option.some.injEq, Prod.mk.injEq] at h
                    obtain ⟨hr', _⟩ := h; exact absurd hr' (by simp)
          | state ℓ0 s0 =>
              simp only [Handler.label] at h
    ...
    · -- RAISED PART (U3 seam-3). Mirrors the U2 `sim` raised arm on the `Config.run`/`dispatchRun` side.
      ...
      | perform cap op2 v2 =>
          ...
          have close : ∀ (o : Bang.OpId),
            ...
            · -- NoResume K n2 o: a resolved frame is throws (abort), or fails the op (the resuming kind is
              ...
              | custom ℓ' p cl =>
                  -- REAL custom dispatch (ADR-0085 Stage 4): the frame FAILS the op iff its clause list has no
                  -- entry for `o` — exactly the custom-miss the raise implies. `splitAtId_custom_value` reads
                  -- the resolved frame's `(p, cl)` off `ctxCustoms`; `hcus` gives the clause miss ⇒ handlesOp false.
                  left
                  have hcv : (ctxCustoms K).get? n2 = some (p, cl) := splitAtId_custom_value hsp
                  rcases hcus with hmiss | hclmiss
                  · rw [hcv] at hmiss; exact absurd hmiss (by simp)
                  · have : cl.find? (·.1 == o) = none := hclmiss p cl hcv
                    simp only [Handler.label, Bang.handlesOp, this, Option.isSome_none, Bool.and_false]
            ...
            · by_cases hopt : isTxnOp op2 = true
              ...
              · rw [Bool.not_eq_true] at hopt
                ...
                | some pcls =>
                    ...
                    | some clause =>
                        -- SERVICE + the CLAUSE BODY RAISES (deep-handler propagation): evalD ran the clause
                        -- body against the SAME store and it raised. Recurse via `ihR` on the clause body,
                        -- bridged by `dispatch_custom` (K/σ/τ/κ unchanged). Mirror of the term-part custom-service.
                        simp only [hcl] at h
                        have hgc : (ctxCustoms K).get? n2 = some (p, cls) := by rw [← hCK]; exact hck
                        obtain ⟨Kᵢ, ℓ', Kₒ, hsp⟩ := splitAtId_of_ctxCustoms_get hFresh.2.2.1 hgc
                        have hlab : ℓ' = ℓ2 := by
      ...
      | force a =>
          ...
          | vthunk M =>
              ...
              obtain ⟨hpair, kR⟩ := ihR M g σ τ κ n op v g' σ' τ' κ' h K hCtx hTtx hCK
                ⟨hWfK, hWfM.force_thunk⟩ hWfσ hWfτ
                (capLabelCoh_step _ _ hFresh hCoh hstep) (freshCfg_step _ _ hFresh hstep)
      ...
      | case a b d =>
          ...
          | inl sv =>
              ...
              have hWfvSv : WfCustomVal sv := fun op hop => hWfM op (by
                simp only [customOpsC, customOpsV]; exact List.mem_append_left _ (List.mem_append_left _ hop))
              obtain ⟨hpair, kR⟩ := ihR (Comp.subst sv b) g σ τ κ n op v g' σ' τ' κ' h K hCtx hTtx hCK
                ⟨hWfK, hWfM.case_left.subst hWfvSv⟩ hWfσ hWfτ
                (capLabelCoh_step _ _ hFresh hCoh hstep) (freshCfg_step _ _ hFresh hstep)
              exact ⟨hpair, fun fuel r hr => by
                obtain ⟨F', hF'⟩ := kR fuel r hr
                exact ⟨F'+1, by simp only [Bang.Config.run, Source.step]; exact hF'⟩⟩
          | inr sv =>
              ...
              obtain ⟨hpair, kR⟩ := ihR (Comp.subst sv d) g σ τ κ n op v g' σ' τ' κ' h K hCtx hTtx hCK
                ⟨hWfK, hWfM.case_right.subst hWfvSv⟩ hWfσ hWfτ
                (capLabelCoh_step _ _ hFresh hCoh hstep) (freshCfg_step _ _ hFresh hstep)
              exact ⟨hpair, fun fuel r hr => by
                obtain ⟨F', hF'⟩ := kR fuel r hr
                exact ⟨F'+1, by simp only [Bang.Config.run, Source.step]; exact hF'⟩⟩
      ...
      | split a b =>
          ...
          | pair sv sw =>
              ...
              have hWfvSv : WfCustomVal sv := fun op hop => hWfM op (by
                simp only [customOpsC, customOpsV]; exact List.mem_append_left _ (List.mem_append_left _ hop))
              have hWfvSw : WfCustomVal sw := fun op hop => hWfM op (by
                simp only [customOpsC, customOpsV]; exact List.mem_append_left _ (List.mem_append_right _ hop))
              have hWfFocus : WfCustomComp (Comp.subst sv (Comp.subst (Val.shift sw) b)) :=
                (hWfM.split_body.subst (show WfCustomVal (Val.shift sw) from fun op hop =>
                  hWfvSw op (customOpsV_shift ▸ hop))).subst hWfvSv
              obtain ⟨hpair, kR⟩ := ihR (Comp.subst sv (Comp.subst (Val.shift sw) b)) g σ τ κ n op v g' σ' τ' κ' h
                K hCtx hTtx hCK ⟨hWfK, hWfFocus⟩ hWfσ hWfτ
                (capLabelCoh_step _ _ hFresh hCoh hstep) (freshCfg_step _ _ hFresh hstep)
              exact ⟨hpair, fun fuel r hr => by
                obtain ⟨F', hF'⟩ := kR fuel r hr
                exact ⟨F'+1, by simp only [Bang.Config.run, Source.step]; exact hF'⟩⟩
      ...
      | letC M0 N =>
          ...
          | some oM =>
            ...
            | (.raised ℓ' op' w, g1, σ1, τ1, κ1), h =>
                simp only [Option.bind_some, Option.some.injEq, Prod.mk.injEq, Outcome.raised.injEq] at h
                obtain ⟨⟨rfl, rfl, rfl⟩, rfl, rfl, rfl, rfl⟩ := h
                obtain ⟨⟨hCr, hTr, hKr, hWfvW, hWfσr, hWfτr, hCohr, hFreshr, hNRr⟩, kR⟩ :=
                  ihR M0 g σ τ κ ℓ' op' w g1 σ1 τ1 κ1 hM (Frame.letF N :: K) hCletF hTletF hKletF
                    ⟨hWfK.cons_noncustom (by intro n ℓ p cls; simp), hWfM.letC_left⟩ hWfσ hWfτ hCletFcoh hFletF
                have hcne : ctxNetEffect (Frame.letF N :: K) σ1 τ1 = Frame.letF N :: ctxNetEffect K σ1 τ1 :=
                  ctxNetEffect_cons_nonframe σ1 τ1 (by intro n ℓ s; simp) (by intro n ℓ Θ; simp)
                ...
                have hKr' : CCtxCorr κ1 (ctxNetEffect K σ1 τ1) := by
                  unfold CCtxCorr at hKr ⊢
                  rw [hKr, ctxCustoms_ctxNetEffect, ctxCustoms_ctxNetEffect]; rfl
                ...
                refine ⟨⟨hCr', hTr', hKr', hWfvW, hWfσr, hWfτr, hCohr', hFreshr', hNRr'⟩, fun fuel r hr => ?_⟩
            ...
            | (.term (.ret v0), g1, σ1, τ1, κ1), h =>
                simp only [Option.bind_some] at h
                obtain ⟨⟨hCM, hTM, hKM, hWftM, hWfσM, hWfτM, hCohR, hFR⟩, kM⟩ :=
                  ihT M0 g σ τ κ (.ret v0) g1 σ1 τ1 κ1 hM (Frame.letF N :: K) hCletF hTletF hKletF
                    ⟨hWfK.cons_noncustom (by intro n ℓ p cls; simp), hWfM.letC_left⟩ hWfσ hWfτ hCletFcoh hFletF
                have hWfvV0 : WfCustomVal v0 := fun op hop => hWftM op (by simpa only [customOpsC] using hop)
                ...
                have hFsub := freshCfg_step _ _ hFR hpop
                obtain ⟨⟨hCr, hTr, hKr, hWfvW, hWfσr, hWfτr, hCohr, hFreshr, hNRr⟩, kR⟩ :=
                  ihR (Comp.subst v0 N) g1 σ1 τ1 κ1 n op v g' σ' τ' κ' h (ctxNetEffect K σ1 τ1) hCM' hTM' hKM'
                    ⟨hWfK.ctxNetEffect σ1 τ1, hWfM.letC_right.subst hWfvV0⟩ hWfσM hWfτM hCsub hFsub
      ...
      | app M0 v0 =>
          ...
          | some oM =>
            ...
            | (.raised ℓ' op' w, g1, σ1, τ1, κ1), h =>
                ...
                obtain ⟨⟨rfl, rfl, rfl⟩, rfl, rfl, rfl, rfl⟩ := h
                obtain ⟨⟨hCr, hTr, hKr, hWfvW, hWfσr, hWfτr, hCohr, hFreshr, hNRr⟩, kR⟩ :=
                  ihR M0 g σ τ κ ℓ' op' w g1 σ1 τ1 κ1 hM (Frame.appF v0 :: K) hCappF hTappF hKappF
                    ⟨hWfK.cons_noncustom (by intro n ℓ p cls; simp), hWfM.app_body⟩ hWfσ hWfτ hCappFcoh hFappF
            ...
            | (.term (.lam N), g1, σ1, τ1, κ1), h =>
                simp only [Option.bind_some] at h
                obtain ⟨⟨hCM, hTM, hKM, hWftM, hWfσM, hWfτM, hCohR, hFR⟩, kM⟩ :=
                  ihT M0 g σ τ κ (.lam N) g1 σ1 τ1 κ1 hM (Frame.appF v0 :: K) hCappF hTappF hKappF
                    ⟨hWfK.cons_noncustom (by intro n ℓ p cls; simp), hWfM.app_body⟩ hWfσ hWfτ hCappFcoh hFappF
                have hWfN : WfCustomComp N := fun op hop => hWftM op (by simpa only [customOpsC] using hop)
                have hWfvV0 : WfCustomVal v0 := hWfM.app_arg
                have hCM' := CtxCorr_ctxNetEffect_nonframe (by intro ℓ s; simp) (by intro ℓ Θ; simp) hCM
                have hTM' := CtxTxnCorr_ctxNetEffect_nonframe (by intro ℓ s; simp) (by intro ℓ Θ; simp) hTM
                have hKM' : CCtxCorr κ1 (ctxNetEffect K σ1 τ1) := by
                  unfold CCtxCorr at hKM ⊢
                ...
                obtain ⟨⟨hCr, hTr, hKr, hWfvW, hWfσr, hWfτr, hCohr, hFreshr, hNRr⟩, kR⟩ :=
                  ihR (Comp.subst v0 N) g1 σ1 τ1 κ1 n op v g' σ' τ' κ' h (ctxNetEffect K σ1 τ1) hCM' hTM' hKM'
                    ⟨hWfK.ctxNetEffect σ1 τ1, hWfN.subst hWfvV0⟩ hWfσM hWfτM hCsub hFsub
                rw [ctxNetEffect_ctxNetEffect] at hCr hTr hKr hCohr hFreshr hNRr
                refine ⟨⟨hCr, hTr, hKr, hWfvW, hWfσr, hWfτr, hCohr, hFreshr, hNRr⟩, fun fuel r hr => ?_⟩
                obtain ⟨F2, hF2⟩ := kR fuel r (by rw [ctxNetEffect_ctxNetEffect]; exact hr)
      ...
      | handle h0 M0 =>
          ...
          cases h0 with
          | custom ℓ0 p0 cls0 =>
              -- custom handle FORWARDS an uncaught raise (ADR-0085 Stage 4; the `customForwardsRaise` witness):
              -- MINT `handleF g (custom ℓ0 p0 cls0)`, push (g ↦ (p0,cls0)) on κ, run M' = subst (vcap g ℓ0) M0;
              -- the body raises and custom NEVER catches (only throws does), so the raise propagates, popping the
              -- custom κ entry (κ1.tail). Custom carries no σ/τ payload ⇒ CtxCorr/CtxTxnCorr pass through.
              -- Mirror of the state raised-forward (below), on the κ side.
              simp only [Handler.label] at h
              ...
              have hWfInstall : WfCustomOps (Frame.handleF g (Handler.custom ℓ0 p0 cls0) :: K) :=
                hWfK.cons_custom hInsCl hInsP
              have hWfMbody : WfCustomComp (Comp.subst (Val.vcap g ℓ0) M0) := hWfM.handle_body.subst WfCustomVal.vcap
                ...
                | (.raised ℓ' op' w, g1, σ1, τ1, κ1), h =>
                    ...
                    obtain ⟨⟨hCr, hTr, hKr, hWfvW, hWfσr, hWfτr, hCohr, hFreshr, hNRr⟩, kR⟩ :=
                      ihR (Comp.subst (Val.vcap g ℓ0) M0) (g+1) σ τ (κ.push g p0 cls0) ℓ' op' w g1 σ1 τ1 κ1 hM
                        (Frame.handleF g (Handler.custom ℓ0 p0 cls0) :: K) hCinstall hTinstall hKinstall
                        ⟨hWfInstall, hWfMbody⟩ hWfσ hWfτ hCohInstall hFreshInstall
                    -- custom carries no state/txn ⇒ net-effect context pops the frame directly.
                    have hnetEq : ctxNetEffect (Frame.handleF g (Handler.custom ℓ0 p0 cls0) :: K) σ1 τ1
                        = Frame.handleF g (Handler.custom ℓ0 p0 cls0) :: ctxNetEffect K σ1 τ1 :=
                      ctxNetEffect_cons_nonframe σ1 τ1 (by intro n ℓ s; simp) (by intro n ℓ Θ; simp)
                    have hCpop := CtxCorr_ctxNetEffect_nonframe (by intro n ℓ s; simp) (by intro n ℓ Θ; simp) hCr
                    have hTpop := CtxTxnCorr_ctxNetEffect_nonframe (by intro n ℓ s; simp) (by intro n ℓ Θ; simp) hTr
                    have hKpop : CCtxCorr κ1.tail (ctxNetEffect K σ1 τ1) :=
                      CCtxCorr_pop_custom (hnetEq ▸ hKr)
                    ...
                    refine ⟨⟨hCpop, hTpop, hKpop, hWfvW, hWfσr, hWfτr, hCohr', hFreshr', hNRr'⟩, fun fuel r hr => ?_⟩
                    -- custom NEVER handles a perform op (only its OWN clauses, which are user ops — never the
                    -- raise's op if it reaches here): handlesOp (custom) fails, so the raise strips the frame.
                    have hhof : ℓ' = g → Bang.handlesOp (Handler.custom ℓ0 p0 cls0)
                        (Handler.label (Handler.custom ℓ0 p0 cls0)) op' = false := by
                      intro hgl; subst hgl
                      rcases hNRr [] (Handler.custom ℓ0 p0 cls0) (ctxNetEffect K σ1 τ1)
          ...
          | state ℓ0 s0 =>
              ...
              | some oM =>
                ...
                | (.raised ℓ' op' w, g1, σ1, τ1, κ1), h =>
                    ...
                    have hKr' : CCtxCorr κ1 (ctxNetEffect K σ1.tail τ1) := by
                      unfold CCtxCorr at hKr ⊢
                      rw [hKr, ctxCustoms_ctxNetEffect, ctxCustoms_ctxNetEffect]; simp only [ctxCustoms]
                    rw [hnetEq] at hCohr hFreshr hNRr
                    ...
                    have hNRr' : NoResume (ctxNetEffect K σ1.tail τ1) ℓ' op' := by
                      ...
                      · subst hℓg; intro Kᵢ h Kₒ hsp
                        exact absurd hsp (by rw [splitAtId_none_of_capsBelow hcbpop]; simp)
                      · exact noResume_strip_cons (by intro h0 he; exact hℓg ((Frame.handleF.inj he).1.symm)) hNRr
                    refine ⟨⟨hCpop, hTpop, hKr', hWfvW, hWfσr.tail, hWfτr, hCohr', hFreshr', hNRr'⟩,
                      fun fuel r hr => ?_⟩
                    ...
                    · simp only [if_neg hk, Option.some.injEq, Prod.mk.injEq, Outcome.raised.injEq] at h
                      ...
                      have hNRr' : NoResume (ctxNetEffect K σ1 τ1) ℓ' op' := by
                        ...
                        · subst hℓg; intro Kᵢ h Kₒ hsp
                          exact absurd hsp (by rw [splitAtId_none_of_capsBelow hcbpop]; simp)
                        · exact noResume_strip_cons (by intro h0 he; exact hℓg ((Frame.handleF.inj he).1.symm)) hNRr
                      refine ⟨⟨hCpop, hTpop, hKr', hWfvW, hWfσr, hWfτr, hCohr', hFreshr', hNRr'⟩, fun fuel r hr => ?_⟩
          ...
          | transaction ℓ0 Θ =>
              ...
              | some oM =>
                ...
                | (.raised ℓ' op' w, g1, σ1, τ1, κ1), h =>
                    ...
                    have hKr' : CCtxCorr κ1 (ctxNetEffect K σ1 τ1.tail) := by
                      unfold CCtxCorr at hKr ⊢
                      rw [hKr, ctxCustoms_ctxNetEffect, ctxCustoms_ctxNetEffect]; simp only [ctxCustoms]
                    rw [hnetEq] at hCohr hFreshr hNRr
                    ...
                    have hNRr' : NoResume (ctxNetEffect K σ1 τ1.tail) ℓ' op' := by
                      ...
                      · subst hℓg; intro Kᵢ h Kₒ hsp
                        exact absurd hsp (by rw [splitAtId_none_of_capsBelow hcbpop]; simp)
                      · exact noResume_strip_cons (by intro h0 he; exact hℓg ((Frame.handleF.inj he).1.symm)) hNRr
                    refine ⟨⟨hCpop, hTpop, hKr', hWfvW, hWfσr, hWfτr.tail, hCohr', hFreshr', hNRr'⟩,
                      fun fuel r hr => ?_⟩
