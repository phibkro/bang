...
/-! ### The renaming reduction lemmas (`renameV`/`renameC`/`renameH`/`renameF`/`renameK` per-constructor
reductions + `splitAtId_rename`). The DYNAMIC keystone that once sat here (`step_rename`/`run_rename`/
`run_rename_converges` + `bumpσ`/`rename*_capsBelow` fixpoints) was DELETED (task #16, R-1): the route-1
`crelK_ret` refactor (ADR-0058) bridges the handleF-pop decomposition directly ("no `Canonical`, no
`CapsBelow`, no `run_bump`"), so the chain became dead code carrying the last `dispatchOn_rename`
custom-arm gap (the one it carried). These base reduction lemmas are retained (`splitAtId_rename` etc.
are static §1/§3). -/
...
/-! ### §5.x — the `renameCfg`/`step_rename`/`run_rename` keystone chain was DELETED (task #16, R-1).

That chain (`dispatchOn_rename`→`idDispatch_rename`→`step_rename`→`run_rename`→`run_rename_converges`
→`run_bump_converges`) plus its `bumpσ`/`rename*_capsBelow`-fixpoint helpers became DEAD CODE when
the route-1 `crelK_ret` refactor (§5.2′ `CrelK_head_step`, ADR-0058) started bridging the handleF-pop
decomposition directly — "no `Canonical`, no `CapsBelow`, no `run_bump`" (see the `CrelK` route-1 note).
The chain had zero live callers; it carried the last `dispatchOn_rename` custom-arm gap. Deleted
whole rather than threaded (dead code with an open gap is debt; history is in git). The `CapsBelow`
PREDICATE family below is KEPT — it still feeds `Canonical` (live in `BinaryLR`/`Soundness`). -/
