/-
  ...
  ── TASK #82 / Phase 1b NOTE (the dead/off-path verdict): ADR-0063 routed v1 soundness through PURE
  typing-preservation (`type_safety'`), NOT this route-β diagonal. This file is reached by NO gated
  headline — the LIVE caps/freshness layer it once hosted was EXTRACTED to `Bang/Freshness.lean`, and
  `Bang/CapCoh.lean` now imports THAT, so the `Audit→CalcVM→CapCoh→Model` edge is SEVERED and `Model`
  has left the gated closure. What remains here (the typeless/graded `LWS*` liveness engine + the
  `NonEscape` diagonal) is consumed ONLY by the out-of-closure refutation witnesses
  (`*Refute.lean`, `WsCfgInterfaceProbe.lean`). See the team-lead deliverable: full removal is a
  SEPARATE decision (it deletes committed do-not-weaken witnesses), so it is left isolated + flagged.
-/
module
...
-- task #82 Phase 1b: the caps/freshness layer lives here now.
...
-- Module reveal (Phase 1a). `@[expose] public section`: the caps/freshness layer that
-- CapCoh unfolded was extracted to `Bang/Freshness.lean` (task #82); the `LWS*` engine +
-- diagonal that remain are reached by no gated headline (only out-of-closure witnesses), so
-- the reveal here no longer crosses into the gated closure.
...
/-! ## §2 — the concrete invariant `WellScoped`: every `vcap` resolves.
    (The cap-collection `capsV`/`capsC`/`capsH`/`capsK` + `ResolvesLabel` it builds on now live
    in `Bang/Freshness.lean`; imported above.) -/
...
-- `VcapFree` (the no-raw-`vcap` elaborator invariant) moved to `Bang/Freshness.lean` (task #82);
-- available here via the `public import Bang.Freshness` above.
