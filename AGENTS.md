...
- **Calculated VM (started):** `oracle-lean/Bang/Calc.lean` — K2, increments **1 (arithmetic `val/add/mul`)** and **2 (`let`/`var`, de Bruijn — the machine gains a runtime environment + `LOOKUP/BIND/UNBIND`)** done. `compile`/`exec`/`Instr` are *derived* from the denotational `eval`; `exec ∘ compile ≡ eval` is **proven, no `sorry`** (`exec_compile`, `compile_correct`). Approach & staging: **ADR-0009** (extrinsic; grown one constructor at a time).
- **Harness:** `Bang/EvalJson.lean` adds `{"op":"eval",…}` (interpreter) and `{"op":"exec",…}` (calculated machine; `srcFromExpr` resolves names→de Bruijn); `harness/` drives an independent TS candidate (`src/eval-candidate.ts`) on 10 eval goldens + pure-core fuzz, and diff-tests the machine vs `eval` on arithmetic+let/var (`test/calc.test.ts`). **24 tests green.**
...
- **Next (grow the calculation, ADR-0009 staging):** `if` (deferred until a Bool/value story lets it diff-test against `Bang.Eval` — it branches on a `Bool` ADT, not an Int) → `force`/application (closures, BANG's distinctive kernel) → effects (swap in the effect monad; the free-monad `eval`'s resumption defunctionalizes into the machine). Each increment extends `Instr`/`compile`/`exec` and re-proves, with a harness diff-test.
