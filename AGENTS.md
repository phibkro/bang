...
- **K3 STARTED — effects, general handler machine, HARNESS-GREEN / PROOF-PENDING:** `oracle-lean/Bang/CalcEff.lean` — **general algebraic handlers** (`perform`/`handle`, label-dispatched, nesting, forwarding) over arithmetic+`let`, with **Throws** (zero-shot) as the first operation. The Hutton–Wright exception machine *generalised to labels* falls out: a runtime **handler stack** with `MARK`/`UNMARK`/`THROW` + unwinding. `eval` is **total** (`Outcome = ret │ exc`); the machine is fuel-bounded with a handler stack (`Result = halt │ uncaught`). Diff-tested green (`evaleff` vs `execeff`, `test/calc-eff.test.ts`). `compile_correct` is **`sorry` + plan** (hardest yet: a two-part ret/exc `sim` with an unwinding invariant). Method grounded in the actual papers (Bahr–Hutton 2022 monadic; Hutton–Wright exceptions) — see chat / the file header.
- **Two `sorry`s in the build:** `unify_sound` (K1) and `CalcEff.compile_correct`. The **entire K2 pure core — arithmetic, let/var, CBV closures, CBN+force — is proven.**
