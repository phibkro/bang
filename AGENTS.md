...
- **Calculated VM — higher-order, HARNESS-GREEN / PROOF-PENDING:** `oracle-lean/Bang/CalcHO.lean` — K2 increment **3 (`λ`/application, closures)**. Fuel-bounded, CBV, `Value = vint | vclo Src Env` (source-closures shared with `eval`). The machine runs and is **diff-tested green** vs `eval`; its equivalence `compile_correct` is shipped as **`sorry` + a written proof plan** (a fuel-indexed simulation — the next proof to land), *not* claimed proven. Kept separate so `Calc.lean` stays fully proven. Decisions: **ADR-0010**.
- **Harness:** `Bang/EvalJson.lean` ops — `{"op":"eval"}` (interpreter), `{"op":"exec"}` (proven first-order machine), `{"op":"execho"}` (HO machine). `harness/` drives an independent TS candidate (`eval-candidate.ts`) on eval goldens + fuzz, and diff-tests each machine vs `eval` (`test/calc.test.ts`, `test/calc-ho.test.ts`). **31 tests green.**
- **Two documented `sorry`s only** (both with plans, harness-backed): `unify_sound` (K1), `CalcHO.compile_correct` (this increment). Everything else builds clean.
...
- **Next:** (a) **land `CalcHO.compile_correct`** (the fuel-indexed simulation proof — plan in the file); (b) grow the calculation — thunk/`$`force + call-by-name (BANG's actual kernel) → `if` (after a Bool/value story) → effects (swap in the effect monad; the free-monad `eval`'s resumption defunctionalizes into the machine).
