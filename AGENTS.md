...
- **One `sorry` left in the whole build:** `unify_sound` (K1, with a plan). The **entire K2 calculation — arithmetic, let/var, CBV closures, and CBN+force — is proven.**
...
- **Next (grow the calculation — read `docs/notes/k2-calculation-playbook.md` first):** ADTs + `match` (gives a Bool/value story) → `if` (now diff-testable vs `Bang.Eval`) → **effects** (the K3 payoff: swap in the effect monad; the free-monad `eval`'s resumption defunctionalizes into the machine — `State`/`Throws` handlers). Optionally discharge `unify_sound`. The pure-core CBN kernel (`CalcCBN`) is the base the effect stage builds on. The playbook has the fuel-alignment trick, the mutual-induction pattern, the tactic recipes, and the gotchas.
