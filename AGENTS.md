...
- **Calculated VM — call-by-name, HARNESS-GREEN / PROOF-PENDING:** `oracle-lean/Bang/CalcCBN.lean` — K2 increment **4 (thunk/`$`force, call-by-name)**. `CalcHO` with the convention flipped to CBN; `THUNK`/`FORCE` fall out. Because `Bang.Eval` is *also* CBN, the machine matches it **exactly** (laziness included) — diff-tested green (`test/calc-cbn.test.ts`, `{"op":"execcbn"}`). `compile_correct` is **`sorry` + plan** (needs a *mutual* `eval`/`forceV` simulation — see the file + playbook); the next proof to land.
- **Two `sorry`s in the build:** `unify_sound` (K1) and `CalcCBN.compile_correct` (this increment), both documented with plans. The first-order (`Calc`) and CBV (`CalcHO`) calculations are fully proven.
