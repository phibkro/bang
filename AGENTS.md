...
- **One `sorry` left in the whole build:** `unify_sound` (K1). **Everything calculated for K2 (arithmetic → let/var → CBV closures → CBN+force) AND K3's first effect (general handlers, Throws) is proven.**
