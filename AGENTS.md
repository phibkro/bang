...
- **One `sorry` left in the whole build:** `unify_sound` (K1). **Everything calculated so far is proven:** K2 (arithmetic → let/var → CBV closures → CBN+force) AND K3 (general handlers/Throws, State).
