...
| [hostio-echo](hostio-echo/) | **ADR-0104 host-IO wedge** — imports the bundled `std/Io.bang` (`Console` print/readLine); the corpus run installs the SIM handler (deterministic, verified-core); the real+record+replay path is exercised by `tools/test-hostio.sh`, not the corpus | ih |
