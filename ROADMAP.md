...
| ◊4 | **LR foundation** | `lr_sound`, `lr_fundamental`, `zero_usage_erasable` proven (`group_recovers` RETIRED — ADR-0032: false-as-stated + vacuous; rollback is the txn handler, `all_or_nothing_abort`) | `Audit.lean` reports axioms ⊆ {propext, Classical.choice, Quot.sound} for these. `effect_sound` (Q14 trace semantics) → ◊5 |
