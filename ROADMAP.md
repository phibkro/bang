...
| ◊4.5 | **LR ▷-subsystem** · **✓ lr_sound CLOSED mod append crux (`a5464da`, 2026-06-24; append research #10 in flight)** | Re-phrase `Crel`/`Krel`/`Srel` with the **IxFree `∀k≤n` Kripke-monotone** reading; close μ fold/unfold + `up` + resumptive handlers (state/txn) + `krel_refl` / `lr_sound`(arbitrary-C) / `zero_usage_erasable`. Build-confirmed necessary (ADR-0039); plain-Nat relations lack the both-ways monotonicity μ/resume need | `Audit.lean`: `lr_fundamental` / `lr_sound` / `zero_usage_erasable` axioms ⊆ {propext, Classical.choice, Quot.sound} (`sorryAx` gone). `crel_unfold(a)` / `krel_appF_intro` / all infra carry over from ◊4 |
