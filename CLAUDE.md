...
| **Cap GC rep design** (#133 · the rep is nearly FREE (clause closures already runtime `$val`s; de-Bruijn index IS identity — nested=210 falls out structurally) · **the GENUINE wall = fail-loud on ESCAPE**: a naive `$cap` SILENTLY SUCCEEDS on an escaped cap because the GC path discarded the handler chain · the landed generation stamp `n < $liveTop` rejects immediate escape but is only a scalar approximation, NOT exact membership: stale state cap 0 revives after cap 1 mints and emitted Wasm prints 7 where the oracle says `escapedCap` · `WgcCapCode` pins scalar revival vs exact rejection axiom-clean · the escape differential keeps this one known-red and stays load-bearing · exact live-id membership is the separately priced fix door) | `docs/notes/cap-gc-rep-design.md` |
...
| **wgcexec calculation plan** (the full calculated GC machine remains a post-v1 plan derived from **evalD**; the `$env` cons-list/box heap and closure spike remain tractability evidence · **2026-07-19 scope correction:** `$liveTop≡WellCounted<g` as exact membership is REFUTED by stale reentry · first bounded tracer `WgcCapCode` extracts only the fixed helper text and proves live state-cell observations, immediate rejection, and scalar-revival/exact-rejection witnesses axiom-clean · emitted bytes unchanged · this is NOT `compileGC`, a `Comp` machine, Wasm semantics, or pure+state adequacy · exact liveness must land before the real fragment simulation; the differential remains load-bearing) | `docs/notes/wgcexec-calculation-plan.md` |
...
| **Stranger test round 6** (agentic resource-contract inspection · 3 fresh task traces, no synthetic score · all 18 tasks pass · F1 = invalid `query contract` says top-level `ok:true` while nested `typeChecked:false` · F2 = realization IDs churn across selection-only swaps · F3 = resolved human B018 loses its stable code while JSON retains it · the organic-human gate remains OPEN) | `docs/notes/stranger-test-6.md` |
