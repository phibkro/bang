...
| **Memory-management survey** (no memory subsystem — CBPV + the grade family IS one · mutable handler = param-passing (D5 slice over `Dispatch.lean:137`) · closures = the only heap escapers · U grade = 0/1/ω policy dial · host-GC→Perceus→regions ladder · WasmFX ⊥ WasmGC · VERDICT cap-escape ≈ region-escape TRUE at the type level (scoped-cap = `runST`), FALSE at runtime (escapedCap catches caps not data)) | `docs/notes/memory-management-survey.md` |
