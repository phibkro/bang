...
| **Cap GC rep design** (#133 · the rep is nearly FREE (clause closures already runtime `$val`s; de-Bruijn index IS identity — nested=210 falls out structurally) · **the GENUINE wall = fail-loud on ESCAPE**: a naive `$cap` silently succeeds after escape · ADR-0119 replaces the refuted scalar watermark with an exact GC-linked `$liveCaps` stack: mint pushes, gate searches membership, exit pops through its named frame and skipped inner nodes · stale reentry plus all four older escape cases fail loud with zero XFAILs; a positive nested-state control returns 105 · `WgcCapCode` pins the scalar counterexample and exact helper behavior axiom-clean, without general adequacy) | `docs/notes/cap-gc-rep-design.md` |
...
| **wgcexec calculation plan** (the full calculated GC machine remains a post-v1 plan derived from **evalD**; the `$env` cons-list/box heap and closure spike remain tractability evidence · scalar exactness is refuted and retained as a counterexample · ADR-0119's exact GC-linked helper now lands with pop-through exit, bounded axiom-clean membership/trace witnesses, and zero-XFAIL differential evidence · this is NOT `compileGC`, a `Comp` machine, Wasm semantics, or pure+state adequacy · the real fragment simulation remains the next separately priced door) | `docs/notes/wgcexec-calculation-plan.md` |
