...
- **✓ Unit 2 — pure spine** (`Bang/Backend/AbstractMachine.lean`, `158f08d` — identical to the earlier `ae5f1ca`, which
...
> (`Bang/Backend/AbstractMachine.lean`), `exec ∘ compile ≡ eval` proven over the whole feature surface, K2 matrix archived,
...
- ONE unified module (e.g. `Bang/Backend/AbstractMachine.lean`) over graded-CBPV `Comp`.
...
- `Bang/Calc.lean` (base), `Bang/CalcEff.lean` (throws-as-unwinding — the handler template),
  `Bang/Reify/CalcReify.lean` (reification frontier — the DEFERRED shape).
