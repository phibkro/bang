...
> Re-key the step-indexed LR (`Bang/Meta/LR.lean` + `Bang/Meta/BinaryLR.lean`) to the identity-dispatch + global-fresh
...
- **run_plug DONE** (`f88879a`, kernel-engineer `runplug`, worktree `/srv/share/projects/lang-bang-runplug`):
  `run_plug_reshape` PROVEN axiom-clean ⊆ {propext, Quot.sound}, zero sorry — TRANSCRIBE into `Bang/Meta/LR.lean`.
...
- **NO kernel dependency** (the big de-risk): the carrier-subst rides EXISTING `Bang/Core/Soundness.lean` substitution
...
- **PREREQ #2 `liveCapsResolveV_weaken` — the one novel decision left.** The INDEXED form over `hv.weaken k hk A'` FAILS: `HasVTy.weaken`
  ...
  `Bang/Core/Soundness.lean` → expose them (~2-line private-removal, pure typing infra, carrier-agnostic-safe). Then the mutual V/C weaken recursor.
...
The fresh IC (aa10) **won the survey-wide race** and closed the carrier-subst keystone *cleaner* than the relayed plan (Prop proof-irrelevance for weaken; a 3-line thunk-wrap C-twin instead of 21 duplicated arms). **`liveCapsResolve{V,C}_subst_gen` + `…_weaken` are axiom-clean** (independently gate-verified at `da67c2d`: `[propext, Classical.choice, Quot.sound]` / `[propext, Quot.sound]`). Also landed: `HasVTy.subst_gen` (value-subst prereq, ret-wrap, `ff4109f`) + de-privatized `insG`/`insT`/`insG_smul/add` in `Bang/Core/Soundness.lean` (pure grade infra). The canonical `inc5-lr-reindex` was fast-forwarded onto it and **pushed to origin** (was local-only). *(`Bang.Surface` is pre-broken at `ff4109f` — imports only `Operational`, unrelated; a full-build red on Surface is NOT a regression.)*
