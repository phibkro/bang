...
> `evalD_caplabelcoh` (import-DAG-forced: caps-only induction, single-config premise, frozen conclusion, needs NEITHER StepStar NOR CtxCorr) is
> **8/10 cases GREEN** (only `perform`+`handle` remain, mechanical mirrors); U2 stays axiom-clean (no #35 leak). Remaining U3 = fill 2 helper cases
> → thread into `run_evalD` + body transfer (errors from CalcVM.lean:3839) → `evalD_agrees_source` gains `VcapFree`. SoT = PATH §U3 STATUS.
