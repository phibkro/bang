...
> - **whole-tree-green:** **U5** (WASM-hop re-derivation, `Compile.lean`, PATH §U5) · **inc-5** (#14 — LR/Compat re-key → the diagonal; ADR-0056/0057, PATH-inc5) · **inc-7** (#16 — Surface re-key) · then the ADR-0063 **Spec re-point** (`HasConfig→HasConfig'`, `progress`→3-disjunct, `type_safety`→`type_safety'_proof`).
