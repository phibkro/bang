...
> `evalD_caplabelcoh` is **8/10 cases GREEN** (the 8 structural cases); U2 stays axiom-clean (no #35 leak). **★ DESIGN PIVOT (build-grounded):**
> `perform`/`handle` need `CtxCorr` (store↔context alignment) — a *separate* helper carrying it would duplicate `run_evalD`'s resume-alignment (SSoT
> violation), so the coherence conjunct FOLDS INTO `run_evalD`'s existing conclusion (which already proves `CtxCorr ∧ CtxTxnCorr`); one induction
> co-proves both, the 8 case-bodies migrate in. Remaining = the strengthened-conclusion **L-unit** (ATOMIC: breaks every case until re-threaded —
> fresh-budget, don't start half-depleted), fully specified as a 5-step plan in PATH §U3 STATUS, resumable from `d576811` + task #59. SoT = PATH §U3.
