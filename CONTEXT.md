...
> - **whole-tree-green:** **U5a ✅ DONE** (the WASM forward lockstep `exec_wexec_sim`, the ◊5 contribution — `lang-bang-u5wasm` @ `ae2b98e`) · **U5b** (the reverse COMPLETENESS bridge `evalD_complete_gen` — a U2-scale ~70-lemma Sim re-derivation, fresh-budget IC; PATH §U5) · **inc-5** (#14 — LR/Compat re-key → the diagonal; ADR-0056/0057, PATH-inc5) · **inc-7** (#16 — Surface re-key) · then the ADR-0063 **Spec re-point** (`HasConfig→HasConfig'`, `progress`→3-disjunct, `type_safety`→`type_safety'_proof`).
...
>
> **Branch/worktree map:** docs/design on `typed-static-r1`; route-B U3 on `lang-bang-u3bridge` @ `06e3eae`; inc-5 on `inc5-lr-reindex`/`inc5-comp-grind`; `main` = the dynamic-dispatch LR (green-1-sorry, `4c77ba8`). Whole tree RED downstream by design until U5 + inc-5 land. **Surface (ADR-0046/0048):** `Bang.Frontend.NamedCore` writable IR landed (`9452660`); trait/law loop in `Bang/Surface/Trait.lean`; physical tier `git mv`s gated on green — detail in `core-overview.md §6`.
