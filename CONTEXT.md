...
> **★★ CURRENT POSITION (2026-06-28, post-session — THIS LEADS; the `★` blocks below are session HISTORY, superseded, kept for resume detail / git):**
> **inc-6 U3 (the route-B CalcVM→kernel bridge) is COMPLETE + gated** — `run_evalD` + `evalD_agrees_source` are sorry-free, `[propext, Classical.choice, Quot.sound]`,
> on `lang-bang-u3bridge` @ `06e3eae`. **The v1 SOUNDNESS payoff is DONE** (the ADR-0063 escape-reclassification proof is axiom-clean + #35-free; route-B closes it at
> the Spec level). **REMAINING for whole-tree-green** (none gate the Audit axiom-set — `Audit` imports `CalcVM`, not `Compile`): **U5** = the WASM-hop re-derivation
> (`Compile.lean`, a SIBLING to U3, NOT a re-key — 5-step plan in PATH §U5) · **inc-5** (Compat/LR re-index → the diagonal) · **inc-7** (Surface re-key) · then the
> ADR-0063 Spec re-point. SoT = **`paths/PATH-inc6-calcvm-route-b.md`** (§U3 done, §U5 next). The generated proof-state above is stale (`84a6bac`); the generator
> SKIPs until the tree greens. ke-witness-gate is mid-flight gating the route-B refutation witnesses into Audit (non-vacuity SURVEY→TEST).
>
> _(session history below — superseded by CURRENT POSITION, retained for the route-B resume detail; SoT is the PATH):_
...
> HANDLE-defer-recompile lines up with the simulation IH, so route-B's hardest proof question is ANSWERED (no design risk left).**
> **★ U3 UNDERWAY + its WALL SOLVED (2026-06-28, worktree `lang-bang-u3bridge` @ `inc6-u3-bridge` @ `d576811`):** the U3 coherence wall (kernel
> dispatches by IDENTITY, `evalD`'s store is LABEL-keyed) is resolved Route-A — thread a label↔identity coherence FACTOR (`Bang/CapCoh.lean`:
> `WeakCoh`/`CapLabelCoh` + the #35-free `capLabelCoh_step`), NOT re-mirror the machine. **Literature-grounded: AsmFX §7.1 (`role:frontier`,
> `grounds:inc-6`) — their source/machine simulation "essentially expresses a well-scoping invariant" — same move.** The propagation helper
> **★★ inc-6 U3 COMPLETE (`06e3eae`, manager-gated independently):** route-B `run_evalD` is **FULLY SORRY-FREE** and `evalD_agrees_source` is **TRUE** — the
> route-B CalcVM↔kernel agreement is PROVEN. `#print axioms` run_evalD/evalD_agrees_source/sim/compile_correct all `[propext, Classical.choice, Quot.sound]`,
> ZERO sorryAx, NO #35. Three units closed: perform-term (verified 4/4 + axiom-certified) · handle-term (3 kinds) · raised (Route A `NoResume` conjunct,
> after Route B build-disproven). **REMAINING for whole-tree-green (NOT gating Audit, which imports CalcVM not Compile):** `Bang/Compile.lean` stale
> **U5 = the WASM-hop RE-DERIVATION** (NOT a re-key — build-diagnosed: `Compile.lean` is route-A's WASM machine vs route-B CalcVM, 103 errors structural; a
> SIBLING to U3, 5-step plan banked in PATH §U5) → then ADR-0063 Spec re-point + inc-5 (Compat) confluence. Worktree `lang-bang-u3bridge` @ `06e3eae`. SoT = PATH §U3/U5.
>
> _(prior milestone, superseded:)_ **ccd6c60:** route-B `run_evalD` was **ONE sorry (raised) from complete** — the reshape + 8
> structural cases + **perform-term (the crux) + handle-term (all 3 handler kinds + throws caught-raise) all CLOSED.** `#print axioms`:
> `sim`/`compile_correct` axiom-clean (U2 no-regression); `run_evalD` `sorryAx` = raised ONLY; 0 wsCfg/#35 refs (structural). **★ FOUNDATION
> AXIOM-CERTIFIED + independently VERIFIED:** the parallel crux-verifier found perform-term 4/4 logically sound (the typing-by-label/dispatch-by-identity
> gap bridged non-vacuously by `capLabelCoh_perform_label`; StratFresh no-shadow by contradiction), AND direct `#print axioms` on ALL supporting lemmas
> (capLabelCoh_step/perform_label/initial, freshCfg_step, capCoh_idDispatch, dispatch_state_*/txn, splitAtId_of_ctxStates_get) = sorryAx-FREE, NO #35.
> **REMAINING = raised (the LAST sorry), carries a statement-level REFUTE-WATCH** (provisional `CapLabelCoh (g', _, ret v)` conclusion — wants a fresh
> alert IC). Resumable from `ccd6c60` + task #59. SoT = PATH §U3.
> Remaining (U2) = mechanical-intricate INCREMENTAL finish (handle-throws/transaction mirror · raised · compile_correct · Agree → the U2 gate),
