...
> **★ f6b0c35 MILESTONE (manager-gated independently):** the strengthened-conclusion reshape + all 8 structural term cases + **perform-term (get/put/txn
> — the conceptual crux) CLOSED route-B.** `#print axioms`: `sim`/`compile_correct` stay axiom-clean (U2 no-regression); `run_evalD` `sorryAx` isolated to
> EXACTLY 2 sites (handle-term 3985 + raised 4056), **0 wsCfg/#35 refs in the region → NO #35 leak (structural, not grep).** The route-A coherence design
> reassembles dispatch end-to-end (store-read → existence lemma → label-match → `CapResolves` → `dispatch_state_*`). **⚠ SCOPE grew (build-confirmed):**
> inc-6 U3 = perform-term ✓ + **handle-term (large, pre-route-B rewrite, next) + raised (large, after)** — 3 substantial units, not 1. Each fresh-budget.
> Raised carries a statement-level REFUTE-WATCH (provisional `CapLabelCoh (g', _, ret v)` conclusion). Resumable from `f6b0c35` + task #59. SoT = PATH §U3.
