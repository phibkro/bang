...
_Generated by `tools/gen-proof-state.py --lean-root <…>`. Proof-state at `96346ff`._
...
> **★★★★★★★ FIRST TRACER BULLET LANDED (2026-06-30) — `96346ff` on `main`, pushed; GitHub #1 (ADTs) CLOSED. THIS LEADS.** Sums + products surface END-TO-END from source text: `Left(e)`/`Right(e)` + `match s { Left(x) -> … , Right(y) -> … }` (sum, arms order-independent) and `(a,b)` + `let (a,b) = p in …` (product) — Gleam/Rust-leaning, mapping 1:1 to the kernel's flat `case`/`split`. **No kernel change, no ADR, outside the verification spine** — pure `Bang/Frontend/Surface.lean` (AST+lower+parse) + corpus; compiled `#guard`s green (Surface Stage 2d, Examples A12/A13); `just verify` passes. Methodology VALIDATED — the surface rode the kernel as-is, no extension needed (invariant #5 as workflow, confirmed). Deferred → GitHub **#23** (named variants `Some`/`Ok` + type declarations: needs a decl layer, bigger than the anonymous core). Parser fix banked: fuel `tokens+1`→`4·tokens+1` (each nesting level is a 3-call descent that may consume one token; `Left(7)` overflowed — deep parens were a latent pre-existing case). **NEXT: operator's pick from milestone #1** — recursive-data (#2, composes with #23) · named/dynamic caps (#3) · arithmetic (#4) · effect-types (#5) · compiled-path (#6).
