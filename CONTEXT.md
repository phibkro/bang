...
> - **whole-tree-green:** **U5a ✅ · U5b-pure ✅ gated** (see lead) · **U5b-handler** (#65 — DEFERRED, fully de-risked; ~1000-line `run_evalD`-inverted handler-completeness grind, resume from `inc6-u5b-handler` scratch trailer; NOT a soundness gap) · **inc-5** (#14 — LR/Compat re-key → the diagonal; ADR-0056/0057, PATH-inc5) · **inc-7** (#16 — Surface re-key) · then the ADR-0063 **Spec re-point** (`HasConfig→HasConfig'`, `progress`→3-disjunct, `type_safety`→`type_safety'_proof`).
...
>
> **Branch/worktree map:** docs/design on `typed-static-r1`; route-B U3 on `lang-bang-u3bridge` @ `06e3eae`; **U5 on `inc6-u5-wasm` @ `fd7a8d2` (U5a+U5b-pure, gated) → `inc6-u5b-handler` @ `4e52e28` (U5b-handler de-risk, scratch-only, off `fd7a8d2`)**; inc-5 on `inc5-lr-reindex`/`inc5-comp-grind`; `main` = the dynamic-dispatch LR (green-1-sorry, `4c77ba8`). Whole tree RED downstream by design until the U5b-handler grind + inc-5 land (the route-B *soundness* headlines are already axiom-clean). **Surface (ADR-0046/0048):** `Bang.Frontend.NamedCore` writable IR landed (`9452660`); trait/law loop in `Bang/Surface/Trait.lean`; physical tier `git mv`s gated on green — detail in `core-overview.md §6`.
